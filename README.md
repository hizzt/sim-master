<a href="https://github.com/techblack/sim-master">
  <img src="https://socialify.git.ci/techblack/sim-master/image?description=1&descriptionEditable=%E9%9D%A2%E5%90%91%20Debian%20%E5%B9%B3%E5%8F%B0%E6%94%AF%E6%8C%81%20SIM%2FeSIM%20%E8%9C%82%E7%AA%9D%E7%9A%84%E5%BC%80%E6%BA%90%20Web%20%E7%AE%A1%E7%90%86%E7%B3%BB%E7%BB%9F&font=Source%20Code%20Pro&logo=https%3A%2F%2Fgithub.com%2Ftechblack%2Fsim-master%2Fblob%2Fmain%2Ffrontend%2Fpublic%2Fsimadmin-logo.svg%3Fraw%3Dtrue&name=1&owner=1&pattern=Floating%20Cogs&theme=Auto" alt="sim-master" />
</a>
...
<div align="center">
  ...
  <div>
    ...
    <a href="./LICENSE">
      <img
        src="https://img.shields.io/github/license/techblack/sim-master?style=flat-square"
      />
    </a >
    <a href="https://github.com/techblack/sim-master/releases">
      <img
        src="https://img.shields.io/github/v/release/techblack/sim-master?style=flat-square"
      />
    </a >
    <a href="https://github.com/techblack/sim-master/releases">
      <img
        src="https://img.shields.io/github/downloads/techblack/sim-master/total?style=flat-square"
      />  
    </a >
  </div>
...
# sim-master - SIM/eSIM 中枢

sim-master（运行时兼容名称为 SimAdmin）是一套面向 Debian 蜂窝 CPE、随身 WiFi、软路由类设备的 SIM/eSIM、蜂窝网络、短信、DDNS 和系统状态管理系统。

快速部署请阅读 [`Readme.md`](Readme.md)，其中包含 Docker Compose、GHCR 镜像和传统 systemd 安装说明。
...
| 模块管理 | `/modems` | 自动发现多个 ModemManager 模块，显示端口、网卡、SIM 与连接状态，并选择当前管理模块 |
| SIM 卡管理 | `/sim` | 全面展示卡状态、标识信息、解锁次数及存储路径，支持号码与短信中心行内修改；在 eSIM 模式下集成管理与写卡功能 |
| 蜂窝网络 | `/network` | 网络注册、服务小区和邻区、运营商扫描、APN、射频模式、频段锁定、小区锁定状态 |
| VoWiFi | `/vowifi` | 分别诊断 IMS、模块原生 VoWiFi、宽带接入、ePDG DNS、SIM-AKA 与 SWu 隧道状态；驻网请求不发起通话 |
| 网络代理 | `/proxy` | 管理按蜂窝模块网卡绑定的 SOCKS5 / HTTP 代理实例、认证、启停与流量统计 |
...
- 多模块请求上下文：前端选择模块后通过 `X-SimAdmin-Modem-Id` 传递稳定设备标识，现有 modem API 会在请求范围内绑定对应模块，不使用进程级全局切换。
- 蜂窝出口代理：每个 SOCKS5 / HTTP 实例使用 `SO_BINDTODEVICE` 绑定所选模块的 `net` 接口；非回环监听强制启用认证。
- IMS / VoWiFi 状态分层：不会把通用 IMS 配置开关当作 Wi-Fi 承载；只有实际注册与承载证据才会报告 VoWiFi 已连接。
- 主机 SWu 隧道：随包安装 `simadmin-vowifi-helper`，经非蜂窝默认路由向运营商 ePDG 发起 IKEv2/NAT-T，并通过所选模块的 `AT+CSIM` 完成 EAP-AKA；页面可启动、停止并查看隧道地址与 P-CSCF。Child SA 建立不等同于 IMS/SIP 已注册。
- 数据连接开关和漫游策略持久化。
- 飞行模式控制。
...
<a href="https://github.com/techblack/sim-master/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=techblack/sim-master" alt="Contributors" />
</a>
...
- [swu-go](https://github.com/1239t/swu-go)（MIT，SWu IKEv2/EAP-AKA helper）
