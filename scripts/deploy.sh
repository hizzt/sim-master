...
VOWIFI_HELPER_BIN="vowifi-helper/simadmin-vowifi-helper"
...
if [ "$DEPLOY_BACKEND" = true ]; then
    ...
    if [ ! -f "$VOWIFI_HELPER_BIN" ]; then
        echo "❌ VoWiFi SWu helper 不存在，请先运行 ./scripts/build.sh"
        exit 1
    fi
fi
...
if [ "$DEPLOY_BACKEND" = true ]; then
    ...
    adb push "$VOWIFI_HELPER_BIN" "${TARGET_PATH}/simadmin-vowifi-helper"
    adb push "vowifi-helper/THIRD_PARTY_NOTICES.md" "${TARGET_PATH}/THIRD_PARTY_NOTICES.md"
    adb shell "chmod +x ${TARGET_PATH}/simadmin"
    adb shell "chmod +x ${TARGET_PATH}/simadmin-vowifi-helper"
    echo "✅ 后端部署完成"
fi
