...
if [ "$BUILD_BACKEND" = true ]; then
    ...
    VOWIFI_HELPER_PATH="vowifi-helper/simadmin-vowifi-helper"

    require_cmd go "请安装 Go 1.24 或更高版本以构建 VoWiFi SWu helper。"
    echo "📡 构建 VoWiFi SWu helper (linux/arm64)..."
    (
        cd vowifi-helper
        CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go build -trimpath -ldflags="-s -w" -o simadmin-vowifi-helper ./
    )
...
if [ "$SKIP_OTA" = false ] && [ "$BUILD_BACKEND" = true ] && [ "$BUILD_FRONTEND" = true ]; then
    ...
    if [ ! -f "$BINARY_PATH" ]; then
    ...
    elif [ ! -f "$VOWIFI_HELPER_PATH" ]; then
        echo "跳过 OTA: VoWiFi SWu helper 不存在"
        ...
        cp "$VOWIFI_HELPER_PATH" "$OTA_TMP/simadmin-vowifi-helper"
        chmod 755 "$OTA_TMP/simadmin-vowifi-helper"
        cp "vowifi-helper/THIRD_PARTY_NOTICES.md" "$OTA_TMP/THIRD_PARTY_NOTICES.md"
...
EOF
        ...
        echo "打包 OTA..."
        cd "$OTA_TMP"
        tar -czf - meta.json simadmin simadmin-vowifi-helper THIRD_PARTY_NOTICES.md www > "$OLDPWD/$OTA_FILE"
        cd "$OLDPWD"
