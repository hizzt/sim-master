...
REPO="${REPO:-techblack/sim-master}"
...
LPAC_COMPAT_RELEASE_BASE_URL="${LPAC_COMPAT_RELEASE_BASE_URL:-https://github.com/techblack/sim-master/releases/download/lpac}"
...
lpac_url_source() {
  url="$1"
  case "$url" in
    "$LPAC_COMPAT_RELEASE_BASE_URL"/*|https://github.com/techblack/sim-master/releases/download/lpac/*)
      printf '%s\n' "compat"
      ;;
...
main() {
  ...
  if [ -f "${tmp_dir}/pkg/simadmin-vowifi-helper" ]; then
    install -m 0755 "${tmp_dir}/pkg/simadmin-vowifi-helper" "${INSTALL_DIR}/simadmin-vowifi-helper"
  fi
  if [ -f "${tmp_dir}/pkg/THIRD_PARTY_NOTICES.md" ]; then
    install -m 0644 "${tmp_dir}/pkg/THIRD_PARTY_NOTICES.md" "${INSTALL_DIR}/THIRD_PARTY_NOTICES.md"
  fi
