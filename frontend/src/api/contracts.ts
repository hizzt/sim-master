...
export interface ModemListResponse {
  modems: ModemSummary[]
}
...
export interface UiPreferences {
  ...
  refresh_interval_ms: 0 | 1000 | 3000 | 5000 | 10000
  ota_proxy_enabled: boolean
  ota_proxy_preset: 'https://gh-proxy.com/' | 'https://ghproxy.net/' | 'https://githubproxy.cc/' | 'custom'
  ota_custom_proxy: string
}
...
export interface VowifiCallStatus {
  call_id: string
  phone_number: string
  state: string
  sip_code: number
  ...
  rtp_bytes_received: number
  rtp_packets_sent: number
  rtp_bytes_sent: number
  rtcp_packets_received: number
  rtcp_bytes_received: number
  audio_packets_decoded: number
  audio_samples_recorded: number
  audio_recording_bytes: number
  audio_recording_truncated: boolean
  rtp_packets_lost: number
  rtp_packets_out_of_order: number
}

export type VowifiCallAudioFormat = 'wav' | 'pcm_s16le'

export interface VowifiCallAudioStats {
  call_id: string
  codec: string
  sample_rate: number
...
export interface VowifiCallAudio {
  call_id: string
  format: string
  content_type: string
  sample_rate: number
...
export interface ImsStatusResponse {
  ...
  sms_capable: boolean
  ims_enabled?: boolean | null
  volte_enabled?: boolean | null
  vowifi_supported?: boolean | null
  vowifi_enabled?: boolean | null
  transport: 'cellular' | 'wifi' | 'none' | 'unknown'
  modem_model: string
  carrier_config: string
  source: string
  message: string
  ...
  epdg_fqdn: string
  epdg_resolved?: boolean | null
  sim_aka_command_available: boolean
  host_tunnel_supported: boolean
  host_tunnel_reason: string
}

export type VowifiTunnelStage = 'stopped' | 'starting' | 'authenticating' | 'established' | 'stopping' | 'failed'

export interface VowifiReceivedSms {
  id: string
  from: string
  text: string
  received_at: string
  rp_mr: number
  rp_ack_sip_code: number
}

export interface VowifiUpstreamProxyRequest {
  enabled: boolean
  address: string
  username: string
  password?: string
...
export interface VowifiTunnelStatus {
  ...
  outer_transport: string
  inner_tx_packets: number
  inner_rx_packets: number
  pcscf_probe_state: 'not_started' | 'sent' | 'responded' | 'timed_out' | 'failed' | 'unavailable'
  pcscf_reachable: boolean
  pcscf_sip_code: number
  pcscf_probe_sent_at: string
  pcscf_probe_response_at: string
  pcscf_probe_error: string
  ims_registration_state: string
  ims_registered: boolean
  ims_authenticated: boolean
  ims_transport: string
  ims_security_mode: string
  ims_ipsec_established: boolean
  ims_error_class: string
  carrier_preset_id: string
  carrier_overrides_loaded: boolean
  ims_cell_id: string
  ims_cell_id_source: string
  ims_register_profile: string
  ims_user_agent: string
  sms_over_ims_ready: boolean
  sms_tx_path_verified: boolean
  sms_rx_path_verified: boolean
  sms_last_tx_at: string
  sms_last_tx_to: string
  sms_last_tx_text: string
  sms_last_tx_message_id: string
  sms_last_tx_sip_code: number
  sms_last_tx_rp_state: string
  sms_last_tx_rp_cause: number
  sms_last_tx_error: string
  sms_last_rx_at: string
  sms_last_rx_id: string
  sms_last_rx_from: string
  sms_last_rx_text: string
  sms_last_rx_rp_mr: number
  sms_last_rx_rp_ack_sip_code: number
  sms_last_rx_state: string
  sms_last_rx_error: string
  sms_received_messages: VowifiReceivedSms[]
  started_at: string
  updated_at: string
  error: string
...
export type VowifiVerificationState = 'passed' | 'running' | 'incomplete' | 'failed' | 'not_started' | 'not_tested' | 'blocked' | 'unavailable'

export interface VowifiVerificationCheck {
  state: VowifiVerificationState
  evidence: string
  observed_at: string
}

export interface VowifiVerificationStatus {
  enabled: boolean
  verdict: 'enabled' | 'tunnel_only' | 'connecting' | 'failed' | 'disabled'
  summary: string
  swu: VowifiVerificationCheck
  pcscf: VowifiVerificationCheck
  ims: VowifiVerificationCheck
  sms_send: VowifiVerificationCheck
  sms_receive: VowifiVerificationCheck
}

export interface VowifiSmsPathVerificationResult {
