...
const API_BASE = '/api'
export const MODEM_STORAGE_KEY = 'simadmin-selected-modem-id'

function selectedModemHeader(): Record<string, string> {
  const modemId = window.localStorage.getItem(MODEM_STORAGE_KEY)?.trim()
  return modemId ? { 'X-SimAdmin-Modem-Id': modemId } : {}
}
...
class SimAdminCurrentAPI {
  async getModems() {
    return request<ApiResponse<ModemListResponse>>('/modems')
  }

  async getProxies() {
    return request<ApiResponse<ProxyListResponse>>('/proxies')
  }

  async createProxy(payload: ProxyUpsertRequest) {
    return request<ApiResponse<ProxyStatus>>('/proxies', {
      method: 'POST',
      body: JSON.stringify(payload),
    })
  }

  async updateProxy(id: string, payload: ProxyUpsertRequest) {
    return request<ApiResponse<ProxyStatus>>(`/proxies/${encodeURIComponent(id)}`, {
      method: 'PUT',
      body: JSON.stringify(payload),
    })
  }
  ...
  async deleteProxy(id: string) {
    return request<ApiResponse<{ deleted: boolean }>>(`/proxies/${encodeURIComponent(id)}`, {
      method: 'DELETE',
    })
  }

  async startProxy(id: string) {
    return request<ApiResponse<ProxyStatus>>(`/proxies/${encodeURIComponent(id)}/start`, {
      method: 'POST',
    })
  }

  async stopProxy(id: string) {
    return request<ApiResponse<ProxyStatus>>(`/proxies/${encodeURIComponent(id)}/stop`, {
      method: 'POST',
    })
  }
  ...
  async setUiPreferences(preferences: Partial<UiPreferences>) {
    return request<ApiResponse<UiPreferences>>('/ui/preferences/save', {
      method: 'POST',
  ...
  async dialVowifiCall(phoneNumber: string) {
    return request<ApiResponse<VowifiCallStatus>>('/vowifi/call/dial', {
      method: 'POST',
      body: JSON.stringify({ phone_number: phoneNumber }),
      timeoutMs: 30000,
    })
  ...
  async hangupVowifiCall(callId: string) {
    return request<ApiResponse<VowifiCallStatus>>('/vowifi/call/hangup', {
      method: 'POST',
  ...
  async getVowifiCallStatuses() {
    return request<ApiResponse<VowifiCallStatus[]>>('/vowifi/call/status')
  }

  async playVowifiCallAudio(callId: string, audioFormat: VowifiCallAudioFormat, audioBase64: string) {
    return request<ApiResponse<VowifiCallAudio>>('/vowifi/call/audio/play', {
      method: 'POST',
      body: JSON.stringify({
        call_id: callId,
  ...
  async getVowifiCallRecording(callId: string, audioFormat: VowifiCallAudioFormat = 'wav') {
    return request<ApiResponse<VowifiCallAudio>>('/vowifi/call/audio/recording', {
      method: 'POST',
      body: JSON.stringify({ call_id: callId, audio_format: audioFormat }),
      timeoutMs: 30000,
    })
  }

  async getVowifiCallAudioStats(callId: string) {
    return request<ApiResponse<VowifiCallAudio>>('/vowifi/call/audio/stats', {
      method: 'POST',
      body: JSON.stringify({ call_id: callId }),
      timeoutMs: 15000,
    })
  ...
  async startImsRegistration() {
    return request<ApiResponse<ImsStatusResponse>>('/ims/register', {
      method: 'POST',
      timeoutMs: 90000,
    })
  }

  async getVowifiTunnelStatus() {
    return request<ApiResponse<VowifiTunnelStatus>>('/vowifi/tunnel/status')
  }

  async getVowifiVerificationStatus() {
    return request<ApiResponse<VowifiVerificationStatus>>('/vowifi/verification/status')
  }

  async verifyVowifiSmsPath(payload: {
    ...
    return request<ApiResponse<VowifiSmsPathVerificationResult>>('/vowifi/sms/verify', {
      method: 'POST',
      body: JSON.stringify(payload),
      timeoutMs: ((payload.timeout_seconds ?? 60) + 15) * 1000,
    })
  }

  async startVowifiTunnel(proxy?: VowifiUpstreamProxyRequest) {
    return request<ApiResponse<VowifiTunnelStatus>>('/vowifi/tunnel/start', {
      method: 'POST',
  ...
  async stopVowifiTunnel() {
    return request<ApiResponse<VowifiTunnelStatus>>('/vowifi/tunnel/stop', {
      method: 'POST',
      body: JSON.stringify({}),
      timeoutMs: 20000,
    })
