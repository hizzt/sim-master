...
- **系统管理器**：systemd。
- **权限**：需要 root 运行权限。
  - 蜂窝出口代理使用 `SO_BINDTODEVICE` 固定网卡，非 root 部署至少需要为服务保留相应网络能力；默认 systemd 单元以 root 运行。
- **IPC 机制**：system D-Bus。
- **核心依赖包与指令**：
...
| `/opt/simadmin/proxy.json` | 代理实例配置，权限为 `0600`；认证密码不会通过读取 API 返回 |
...
代理实例独立保存在 `/opt/simadmin/proxy.json`。服务启动后会恢复标记为启用的实例，并重新从 ModemManager 解析模块对应的 `net` 接口；模块缺失、网卡缺失或端口占用时不会回退到宿主机默认出口。
