...
use crate::{
    ...
    db::Database,
    vowifi_tunnel::VowifiTunnelLaunchConfig,
};
use crate::{
    models::{
        ...
        DeviceInfoResponse, ImsStatusResponse, ModemListResponse, ModemSummary,
        NetworkInfoResponse, OperatorInfo, OperatorListResponse, RadioMode, RadioModeResponse,
        ServingCell, SetApnRequest, SignalStrengthResponse, SimInfoResponse,
    },
    serial::with_serial,
    system_event::{
...
tokio::task_local! {
    static REQUEST_MODEM_ID: Option<String>;
}

pub async fn scope_request_modem<F, T>(modem_id: Option<String>, future: F) -> T
where
    F: Future<Output = T>,
{
    REQUEST_MODEM_ID.scope(modem_id, future).await
}

fn requested_modem_id() -> Option<String> {
    REQUEST_MODEM_ID
...
pub(crate) async fn list_modem_paths(conn: &Connection) -> zbus::Result<Vec<String>> {
...
fn modem_path_id(path: &str) -> String {
...
fn modem_matches_id(path: &str, props: &InterfaceProperties, id: &str) -> bool {
    path == id
        || modem_path_id(path) == id
        || property_string(props, "DeviceIdentifier").as_deref() == Some(id)
        || property_string(props, "EquipmentIdentifier").as_deref() == Some(id)
}

pub async fn find_modem_path_by_id(conn: &Connection, id: &str) -> zbus::Result<String> {
    let id = id.trim();
    if id.is_empty() {
        return Err(no_modem_error("Modem id is empty"));
    }
    for path in list_modem_paths(conn).await? {
        let props = get_all_properties(conn, &path, MM_MODEM)
            .await
            .unwrap_or_default();
        if modem_matches_id(&path, &props, id) {
            return Ok(path);
        }
    }
    Err(no_modem_error(format!(
        "Selected modem is unavailable: {id}"
    )))
}

pub async fn modem_network_interface_by_id(conn: &Connection, id: &str) -> Result<String, String> {
    let path = find_modem_path_by_id(conn, id)
        .await
        .map_err(|error| error.to_string())?;
    modem_ports(conn, &path)
        .await
        .into_iter()
        .find(|(_, port_type)| *port_type == MM_MODEM_PORT_TYPE_NET)
        .map(|(port, _)| port)
        .ok_or_else(|| format!("Modem {id} does not expose a network interface"))
}

pub async fn get_modems_data(conn: &Connection) -> Result<ModemListResponse, String> {
    let selected_id = requested_modem_id();
    let paths = list_modem_paths(conn)
        .await
        .map_err(|error| error.to_string())?;
    ...
    for (index, path) in paths.into_iter().enumerate() {
        let props = get_all_properties(conn, &path, MM_MODEM)
            .await
            .map_err(|error| error.to_string())?;
        let gpp_props = get_all_properties(conn, &path, MM_MODEM_3GPP)
            .await
            .unwrap_or_default();
        let path_id = modem_path_id(&path);
        let device_identifier = property_string(&props, "DeviceIdentifier").unwrap_or_default();
        let equipment_identifier = property_string(&props, "EquipmentIdentifier")
            .or_else(|| property_string(&gpp_props, "Imei"))
            .unwrap_or_default();
        let id = if !device_identifier.is_empty() {
            device_identifier.clone()
        } else if !equipment_identifier.is_empty() {
            equipment_identifier.clone()
        } else {
            path_id.clone()
        };
        let ports = modem_ports(conn, &path).await;
        let network_interface = ports
            .iter()
            .find(|(_, port_type)| *port_type == MM_MODEM_PORT_TYPE_NET)
            .map(|(port, _)| port.clone())
            .unwrap_or_default();
        let state_value = props.get("State").map(extract_i32).unwrap_or_default();
        let signal_quality = props
            .get("SignalQuality")
            .and_then(|value| <(u32, bool)>::try_from(value.clone()).ok())
            .map(|(quality, _)| quality)
            .unwrap_or_default();
        let sim_present = get_sim_path(conn, &path)
            .await
            .is_ok_and(|sim_path| !sim_path.is_empty() && sim_path != "/");
        let selected = selected_id.as_deref().map_or(index == 0, |selected| {
            modem_matches_id(&path, &props, selected)
        });

        modems.push(ModemSummary {
            id,
            path_id,
            path,
            device_identifier,
            ...
            revision: property_string(&props, "Revision").unwrap_or_default(),
            primary_port: property_string(&props, "PrimaryPort").unwrap_or_default(),
            network_interface,
            state: modem_state_label(state_value).to_string(),
            signal_quality,
            operator_name: property_string(&gpp_props, "OperatorName").unwrap_or_default(),
            sim_present,
            enabled: state_value >= 6,
            connected: state_value == MM_MODEM_STATE_CONNECTED,
            selected,
        });
    }

    Ok(ModemListResponse { modems })
}
...
pub async fn find_modem_path(conn: &Connection) -> zbus::Result<String> {
    if let Some(id) = requested_modem_id() {
        return find_modem_path_by_id(conn, &id).await;
    }
...
async fn at_command_devices(conn: &Connection) -> Result<Vec<String>, String> {
    ...
    let mut devices = ports
        .iter()
        .filter(|(_, port_type)| *port_type == MM_MODEM_PORT_TYPE_AT)
        .map(|(port, _)| modem_device_path(port))
        .collect::<Vec<_>>();
    for (port, _) in &ports {
        let device = modem_device_path(port);
        if looks_like_at_port(port) && !devices.contains(&device) {
            devices.push(device);
        }
    }
    if devices.is_empty() {
        Err("未找到 AT 端口（例如 /dev/wwan0at0）".to_string())
    } else {
        Ok(devices)
    }
}
...
struct QmiImsServices {
    voice_enabled: Option<bool>,
    voice_wifi_enabled: Option<bool>,
...
struct AtImsStatus {
    ...
    volte_enabled: Option<bool>,
}

fn parse_yes_no(value: &str) -> Option<bool> {
    match value.trim().to_ascii_lowercase().as_str() {
        "yes" | "true" | "enabled" | "1" => Some(true),
        "no" | "false" | "disabled" | "0" => Some(false),
        _ => None,
    }
}

fn parse_qmicli_ims_services(output: &str) -> QmiImsServices {
    let mut services = QmiImsServices::default();
    for line in output.lines() {
        ...
        if lower.contains("voice wifi service enabled") {
            services.voice_wifi_enabled = parsed;
        } else if lower.contains("voice service enabled") {
            services.voice_enabled = parsed;
        } else if lower.contains("registration") && lower.contains("enabled") {
            services.registration_enabled = parsed;
        } else if lower.contains("sms service enabled") {
            services.sms_enabled = parsed;
        }
    }
    services
}

fn parse_ims_at_values(output: &str, marker: &str) -> Vec<u8> {
    output
        .lines()
        .find_map(|line| {
...
fn parse_qcfg_ims_status(output: &str) -> AtImsStatus {
    let values = parse_ims_at_values(output, "\"ims\"");
    AtImsStatus {
        // Quectel uses 0=disabled, 1=enabled and 2=automatic.
        enabled: values.first().map(|value| *value != 0),
        // The second value is the read-only IMS registration state.
        registered: values.get(1).map(|value| *value == 1),
        volte_enabled: None,
    }
}

fn parse_qimscfg_ims_status(output: &str) -> AtImsStatus {
    let legacy_values = parse_ims_at_values(output, "\"ims\"");
    let (values, registered_index) = if legacy_values.is_empty() {
        (parse_ims_at_values(output, "\"ims_enable\""), 2)
    } else {
        // Older EC25 firmware exposes `"ims",<mode>,<registration>`.
        (legacy_values, 1)
    };
    AtImsStatus {
        enabled: values.first().map(|value| *value != 0),
        registered: values.get(registered_index).map(|value| *value == 1),
        volte_enabled: None,
    }
}

fn ims_registration_is_active(modem_state: i32, reported: Option<bool>) -> bool {
    modem_state >= MM_MODEM_STATE_ENABLED && reported.unwrap_or(false)
}

fn parse_at_model(output: &str) -> String {
    output
        .lines()
        .map(str::trim)
        .find(|line| {
...
fn at_output_has_payload(output: &str) -> bool {
    output.lines().map(str::trim).any(|line| {
        !line.is_empty()
            && !line.eq_ignore_ascii_case("ok")
            && !line.to_ascii_uppercase().starts_with("AT+")
            && !line.to_ascii_uppercase().contains("ERROR")
    })
}

#[cfg(unix)]
fn usb_model_from_modem_properties(props: &InterfaceProperties) -> Option<String> {
    let device = property_string(props, "Device")?;
    let vendor = fs::read_to_string(format!("{device}/idVendor")).ok()?;
    let product = fs::read_to_string(format!("{device}/idProduct")).ok()?;
    match (
        vendor.trim().to_ascii_lowercase().as_str(),
        product.trim().to_ascii_lowercase().as_str(),
    ) {
        ("2c7c", "0125") => Some("Quectel EC25 family".to_string()),
        _ => None,
    }
}

#[cfg(not(unix))]
fn usb_model_from_modem_properties(_props: &InterfaceProperties) -> Option<String> {
    None
}

async fn query_ims_at(
    conn: &Connection,
    modem_path: &str,
    command: &str,
) -> Result<String, String> {
    let mut last_error = match send_at_via_modem_command(conn, modem_path, command).await {
        Ok(output) if at_output_has_payload(&output) => return Ok(output),
        Ok(_) => "Modem.Command returned no response data".to_string(),
        Err(error) => error,
    };
    let devices = at_command_devices(conn).await?;
    for device in devices {
        match with_serial(async { run_direct_at_command_on_device(device, command).await }).await {
            Ok(output) if at_output_has_payload(&output) => return Ok(output),
            Ok(_) => last_error = "AT port returned no response data".to_string(),
            Err(error) => last_error = error,
        }
    }
    Err(format!("direct AT fallbacks failed: {last_error}"))
}

async fn execute_ims_at(conn: &Connection, modem_path: &str, command: &str) -> Result<(), String> {
    let mut last_error = match send_at_via_modem_command(conn, modem_path, command).await {
        Ok(_) => return Ok(()),
        Err(error) => error,
    };
...
async fn query_qmi_ims_services(
    conn: &Connection,
    modem_path: &str,
) -> Result<QmiImsServices, String> {
    let device = qmi_control_device(conn, modem_path)
        .await
        .ok_or_else(|| "QMI control port is unavailable".to_string())?;
    let args = vec![
        "-p".to_string(),
        "-d".to_string(),
        device,
        "--ims-get-ims-services-enabled-setting".to_string(),
    ];
    let output = with_serial(async {
        run_recovery_command_owned("qmicli", &args, Duration::from_secs(12)).await
    })
    .await?;
    Ok(parse_qmicli_ims_services(&output))
}

fn default_access_interface(excluded_interface: &str) -> String {
    ...
    routes
        ...
        .filter_map(|line| {
            ...
            (columns.len() > 3 && columns[1] == "00000000").then(|| columns[0].to_string())
        })
        .find(|interface| interface != excluded_interface && interface != "lo")
        .unwrap_or_default()
}

fn interface_is_ready(interface: &str) -> bool {
...
fn epdg_fqdn_from_identity(identity: Option<&SimIdentity>) -> String {
    ...
    let digits = identity
        ...
        .collect::<String>();
    let plmn = if matches!(digits.len(), 5 | 6) {
        digits
    } else {
        let imsi = identity
            .imsi
    ...
    let mcc = &plmn[..3];
    let mnc = format!("{:0>3}", &plmn[3..]);
    format!("epdg.epc.mnc{mnc}.mcc{mcc}.pub.3gppnetwork.org")
}

async fn epdg_resolves(fqdn: &str) -> Option<bool> {
    if fqdn.is_empty() {
        return None;
    }
    Some(
        tokio::time::timeout(Duration::from_secs(3), tokio::net::lookup_host((fqdn, 500)))
            .await
            .is_ok_and(|result| result.is_ok_and(|mut addresses| addresses.next().is_some())),
    )
}

async fn sim_aka_at_available(conn: &Connection, modem_path: &str) -> bool {
    if send_at_via_modem_command(conn, modem_path, "AT+CSIM=?")
        .await
        .is_ok()
    {
        return true;
    ...
    for (port, port_type) in modem_ports(conn, modem_path).await {
        if port_type != MM_MODEM_PORT_TYPE_AT {
            continue;
        }
        let device = modem_device_path(&port);
        if with_serial(async { run_direct_at_command_on_device(device, "AT+CSIM=?").await })
            .await
            .is_ok()
        {
            return true;
...
#[cfg(test)]
fn epdg_plmn(fqdn: &str) -> Option<(String, String)> {
    let mut mcc = None;
    let mut mnc = None;
    for label in fqdn.split('.') {
        if let Some(value) = label.strip_prefix("mcc") {
            if value.len() == 3 && value.chars().all(|digit| digit.is_ascii_digit()) {
                mcc = Some(value.to_string());
            }
        }
        if let Some(value) = label.strip_prefix("mnc") {
            if value.len() == 3 && value.chars().all(|digit| digit.is_ascii_digit()) {
                mnc = Some(value.to_string());
            }
        }
    }
    Some((mcc?, mnc?))
...
async fn resolve_epdg_ip(fqdn: &str, prefer_ipv4: bool) -> Result<String, String> {
    let addresses =
        tokio::time::timeout(Duration::from_secs(5), tokio::net::lookup_host((fqdn, 500)))
            .await
            .map_err(|_| format!("Timed out resolving ePDG {fqdn}"))?
            .map_err(|error| format!("Failed to resolve ePDG {fqdn}: {error}"))?
            .collect::<Vec<_>>();
    addresses
        .iter()
        .find(|address| address.ip().is_ipv4() == prefer_ipv4)
        .or_else(|| addresses.first())
        .map(|address| address.ip().to_string())
        .ok_or_else(|| format!("ePDG {fqdn} resolved without an address"))
}

fn format_vowifi_live_cell_id(tac: u32, cell_id: u32) -> String {
    if tac == 0 && cell_id == 0 {
        return String::new();
    }
    format!("{:04X}{:07X}", tac & 0xFFFF, cell_id & 0x0FFF_FFFF)
}

async fn resolve_vowifi_live_cell_id(conn: &Connection) -> String {
    let already_in_airplane_mode = get_airplane_mode(conn)
        .await
        .map(|status| status.enabled)
        .unwrap_or(false);
    let deadline = tokio::time::Instant::now()
        + if already_in_airplane_mode {
            Duration::ZERO
        } else {
    ...
    loop {
        if let Ok(cells) = get_cells_data(conn).await {
            let live_cell_id =
                format_vowifi_live_cell_id(cells.serving_cell.tac, cells.serving_cell.cell_id);
            if !live_cell_id.is_empty() {
                return live_cell_id;
            }
        }
...
pub async fn prepare_vowifi_tunnel_config(
    conn: &Connection,
    db: &Database,
) -> Result<VowifiTunnelLaunchConfig, String> {
    ...
    let modem_props = get_all_properties(conn, &modem_path, MM_MODEM)
        .await
        .map_err(|error| error.to_string())?;
    let path_id = modem_path_id(&modem_path);
    let device_identifier = property_string(&modem_props, "DeviceIdentifier").unwrap_or_default();
    let equipment_identifier =
        property_string(&modem_props, "EquipmentIdentifier").unwrap_or_default();
    let modem_id = if !device_identifier.is_empty() {
        device_identifier
    } else if !equipment_identifier.is_empty() {
        equipment_identifier
    } else {
    ...
    let identity = current_sim_identity(conn)
        .await
        .ok_or_else(|| format!("Unable to read SIM identity for modem {modem_id}"))?;
    let sim_info = get_sim_info_data_with_cache(conn, Some(db))
        .await
        .map_err(|error| format!("Unable to read SIM SMS settings: {error}"))?;
    let live_smsc = query_ims_at(conn, &modem_path, "AT+CSCA?")
        .await
        .map(|output| parse_smsc_from_at_output(&output))
        .unwrap_or_default();
    let smsc = if live_smsc.is_empty() {
        sim_info.sms_center.clone()
    } else {
        cache_smsc_for_identity(db, &identity, &live_smsc, "vowifi_live_at");
        info!(smsc = %live_smsc, "VoWiFi startup SMSC refreshed via AT+CSCA?");
        live_smsc
    };
    let epdg_fqdn = epdg_fqdn_from_identity(Some(&identity));
    if epdg_fqdn.is_empty() {
        return Err("Unable to derive an ePDG name from the selected SIM".to_string());
    }
    let mcc = sim_info.mcc.trim().to_string();
    let mnc = sim_info.mnc.trim().to_string();
    if mcc.is_empty() || mnc.is_empty() {
        return Err("Unable to derive home MCC/MNC from the selected SIM".to_string());
    }

    let modem_interface = ports
        .iter()
        .find(|(_, port_type)| *port_type == MM_MODEM_PORT_TYPE_NET)
        .map(|(port, _)| port.clone())
        .unwrap_or_default();
    let access_interface = default_access_interface(&modem_interface);
    if access_interface.is_empty() || !interface_is_ready(&access_interface) {
        return Err("No active non-cellular default route is available for VoWiFi".to_string());
    }
    let interfaces = crate::utils::read_network_interfaces(None).await?;
    let access = interfaces
        .iter()
        .find(|interface| interface.name == access_interface)
        .ok_or_else(|| format!("Unable to inspect access interface {access_interface}"))?;
    let local_ip = crate::utils::interface_addresses_for_family(
        &access.ip_addresses,
        crate::utils::NetworkAddressFamily::Ipv4,
    )
    .first()
    .map(|address| address.address.clone())
    .ok_or_else(|| format!("Access interface {access_interface} has no usable IPv4 address"))?;
    let epdg_ip = resolve_epdg_ip(&epdg_fqdn, true).await?;
    let live_cell_id = resolve_vowifi_live_cell_id(conn).await;

    Ok(VowifiTunnelLaunchConfig {
        modem_id,
        serial_device,
        access_interface,
        local_ip,
        ...
        live_cell_id,
        smsc,
        phone_number: sim_info.phone_numbers.first().cloned().unwrap_or_default(),
        proxy_enabled: false,
...
/// Read IMS registration state without changing modem configuration.
pub async fn get_ims_status_data(conn: &Connection) -> Result<ImsStatusResponse, String> {
    let modem_path = find_modem_path(conn).await.map_err(|err| err.to_string())?;
    let modem_props = get_all_properties(conn, &modem_path, MM_MODEM)
        .await
        .map_err(|err| err.to_string())?;
    let mut modem_model = property_string(&modem_props, "Model").unwrap_or_default();
    let carrier_config = property_string(&modem_props, "CarrierConfiguration").unwrap_or_default();

    let qmi_services = query_qmi_ims_services(conn, &modem_path).await.ok();
    let mut sources = Vec::new();
    if qmi_services.is_some() {
        sources.push("qmi");
    }

    let mut qimscfg_status = AtImsStatus::default();
    if let Ok(output) = query_ims_at(conn, &modem_path, "AT+QIMSCFG=\"ims\"").await {
        qimscfg_status = parse_qimscfg_ims_status(&output);
        if qimscfg_status.enabled.is_some() {
            sources.push("at-qimscfg-ims");
        }
    }
    if qimscfg_status.enabled.is_none() {
        if let Ok(output) = query_ims_at(conn, &modem_path, "AT+QIMSCFG=\"ims_enable\"").await {
            qimscfg_status = parse_qimscfg_ims_status(&output);
            if qimscfg_status.enabled.is_some() {
                sources.push("at-qimscfg-ims-enable");
            }
        }
    }

    let mut qcfg_status = AtImsStatus::default();
    if let Ok(output) = query_ims_at(conn, &modem_path, "AT+QCFG=\"ims\"").await {
        qcfg_status = parse_qcfg_ims_status(&output);
        if qcfg_status.enabled.is_some() {
            sources.push("at-qcfg");
        }
    }

    let generic_model = modem_model.is_empty()
        || modem_model.eq_ignore_ascii_case("QUECTEL Mobile Broadband Module");
    if generic_model {
        if let Ok(output) = query_ims_at(conn, &modem_path, "AT+CGMM").await {
            let at_model = parse_at_model(&output);
            if !at_model.is_empty() {
                modem_model = at_model;
            }
        }
    }
    if generic_model {
        if let Some(usb_model) = usb_model_from_modem_properties(&modem_props) {
            modem_model = usb_model;
        }
    }

    let voice_enabled = qmi_services
        .as_ref()
        .and_then(|services| services.voice_enabled)
        .or(qcfg_status.volte_enabled);
    let sms_enabled = qmi_services
        .as_ref()
        .and_then(|services| services.sms_enabled)
        .or(qimscfg_status.enabled)
        .or(qcfg_status.enabled);
    let ims_enabled = qmi_services
        .as_ref()
        .and_then(|services| services.registration_enabled)
        .or(qimscfg_status.enabled)
        .or(qcfg_status.enabled);
    let vowifi_enabled = qmi_services
        .as_ref()
        .and_then(|services| services.voice_wifi_enabled);
    let vowifi_supported = if qmi_services
        .as_ref()
        .is_some_and(|services| services.voice_wifi_enabled.is_some())
    {
        Some(true)
    } else {
        None
    };
    // Some Quectel firmware retains the read-only IMS registration bit after
    // the modem is disabled. Never expose that stale value as a live session.
    let modem_state = modem_props
        .get("State")
        .map(extract_i32)
        .unwrap_or_default();
    let registered = ims_registration_is_active(
        modem_state,
        qimscfg_status.registered.or(qcfg_status.registered),
    );
    let transport = if registered { "unknown" } else { "none" };
    let modem_interface = modem_ports(conn, &modem_path)
        .await
        .into_iter()
        .find(|(_, port_type)| *port_type == MM_MODEM_PORT_TYPE_NET)
        .map(|(port, _)| port)
        .unwrap_or_default();
    let access_interface = default_access_interface(&modem_interface);
    let access_ready = interface_is_ready(&access_interface);
    let sim_identity = current_sim_identity(conn).await;
    let epdg_fqdn = epdg_fqdn_from_identity(sim_identity.as_ref());
    let epdg_resolved = epdg_resolves(&epdg_fqdn).await;
    let sim_aka_command_available = sim_aka_at_available(conn, &modem_path).await;
    let helper_available = crate::vowifi_tunnel::helper_is_available();
    let host_tunnel_supported = helper_available && sim_aka_command_available;
    let host_tunnel_reason = if host_tunnel_supported {
        "The SWu IKEv2/EAP-AKA helper and SIM AT authentication are available".to_string()
    } else if !helper_available {
        "The SWu IKEv2/EAP-AKA helper is not installed next to SimAdmin".to_string()
    } else {
        "The selected modem did not accept AT+CSIM required by EAP-AKA".to_string()
    };
    let message = match (registered, ims_enabled, vowifi_supported) {
        (true, _, _) => {
            "IMS registration is active; the firmware does not expose its transport".to_string()
        }
        (false, Some(true), Some(true)) => {
            "IMS and modem-native VoWiFi are enabled, but no IMS registration is active".to_string()
        }
        (false, Some(true), _) => {
            "IMS is enabled, but no Wi-Fi/ePDG transport or IMS registration is active".to_string()
        }
        (false, Some(false), _) => "IMS registration is disabled".to_string(),
        _ => "The modem firmware does not expose IMS registration state".to_string(),
    };

    Ok(ImsStatusResponse {
        registered,
        voice_capable: voice_enabled.unwrap_or(false),
        sms_capable: sms_enabled.unwrap_or(false),
        ims_enabled,
        volte_enabled: voice_enabled,
        vowifi_supported,
        vowifi_enabled,
        ...
        epdg_fqdn,
        epdg_resolved,
        sim_aka_command_available,
        host_tunnel_supported,
        host_tunnel_reason,
    })
}

/// Enable the modem and request IMS registration. This does not originate a call.
pub async fn start_ims_registration(conn: &Connection) -> Result<ImsStatusResponse, String> {
    let modem_path = find_modem_path(conn).await.map_err(|err| err.to_string())?;
    let state = set_modem_enabled(conn, &modem_path, true).await?;
    if state < MM_MODEM_STATE_REGISTERED {
        register_operator_on_modem(conn, &modem_path, "").await?;
    }

    // EC25 variants may query through the `ims_enable` alias while only accepting
    // the legacy `ims` setter, so try the known setters in compatibility order.
    if execute_ims_at(conn, &modem_path, "AT+QIMSCFG=\"ims\",1")
        .await
        .is_err()
        && execute_ims_at(conn, &modem_path, "AT+QIMSCFG=\"ims_enable\",1")
            .await
            .is_err()
    {
        execute_ims_at(conn, &modem_path, "AT+QCFG=\"ims\",1").await?;
    }

    tokio::time::sleep(Duration::from_secs(2)).await;
    get_ims_status_data(conn).await
}
...
mod tests {
    ...
    #[test]
    fn formats_vowifi_cell_id_like_vohive() {
        assert_eq!(
            format_vowifi_live_cell_id(0x11AB, 0x0D9B71FD),
            "11ABD9B71FD"
        );
        assert_eq!(
            format_vowifi_live_cell_id(0x12345, 0x1ABC_DEF0),
            "2345ABCDEF0"
        );
        assert_eq!(format_vowifi_live_cell_id(0, 0), "");
    }

    #[test]
    fn parses_qmi_ims_and_vowifi_services() {
        let output = r#"[/dev/cdc-wdm0] IMS services:
       ...
       Voice WiFi service enabled: no
        ...
        assert_eq!(
            parse_qmicli_ims_services(output),
            QmiImsServices {
                voice_enabled: Some(true),
                voice_wifi_enabled: Some(false),
    ...
    #[test]
    fn parses_quectel_qcfg_ims_status() {
        assert_eq!(
            parse_qcfg_ims_status("AT+QCFG=\"ims\"\r\n+QCFG: \"ims\",1,1\r\nOK"),
            AtImsStatus {
                enabled: Some(true),
        ...
        assert_eq!(
            parse_qcfg_ims_status("+QCFG: \"ims\",0,0"),
            AtImsStatus {
                enabled: Some(false),
    ...
    #[test]
    fn parses_quectel_qimscfg_registration_status() {
        assert_eq!(
            parse_qimscfg_ims_status("+QIMSCFG: \"ims_enable\",1,0,1"),
            AtImsStatus {
                enabled: Some(true),
                registered: Some(true),
                volte_enabled: None,
        ...
        assert_eq!(
            parse_qimscfg_ims_status("+QIMSCFG: \"ims\",1,0"),
            AtImsStatus {
                enabled: Some(true),
                registered: Some(false),
                volte_enabled: None,
    ...
    #[test]
    fn ignores_stale_ims_registration_when_modem_is_disabled() {
        assert!(!ims_registration_is_active(3, Some(true)));
        assert!(ims_registration_is_active(
    ...
    #[test]
    fn rejects_at_echo_without_response_payload() {
        assert!(!at_output_has_payload("AT+QIMSCFG=\"ims\"\r\nOK"));
        assert!(!at_output_has_payload("ERROR"));
        assert!(at_output_has_payload("+QIMSCFG: \"ims\",1,0\r\nOK"));
    }
...
\tRX bytes OK: 1092";
    ...
    #[test]
    fn derives_standard_epdg_fqdn_from_home_plmn() {
        let identity = SimIdentity {
            iccid: String::new(),
            imsi: "234331234567890".to_string(),
            operator_id: "23433".to_string(),
        };
        assert_eq!(
            epdg_fqdn_from_identity(Some(&identity)),
            "epdg.epc.mnc033.mcc234.pub.3gppnetwork.org"
        );
    }

    #[test]
    fn parses_epdg_plmn_for_swu_helper() {
        assert_eq!(
            epdg_plmn("epdg.epc.mnc010.mcc234.pub.3gppnetwork.org"),
            Some(("234".to_string(), "010".to_string()))
        );
        assert_eq!(epdg_plmn("example.invalid"), None);
    }

    #[tokio::test]
    async fn scopes_modem_selection_to_the_current_request() {
        assert_eq!(requested_modem_id(), None);
        let selected =
            scope_request_modem(Some("device-a".to_string()), async { requested_modem_id() }).await;
        assert_eq!(selected.as_deref(), Some("device-a"));
        assert_eq!(requested_modem_id(), None);
    }
}
