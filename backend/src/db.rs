...
impl Database {
    ...
    pub fn new(db_path: PathBuf) -> Result<Self> {
        ...
        // Older builds deduplicated protocol-backed SMS only in application
        // code. Collapse any legacy duplicates before enforcing the marker as
        // a database invariant. NULL markers remain unrestricted for ordinary
        // messages that have no protocol identity.
        conn.execute(
            "DELETE FROM sms_messages
             WHERE pdu IS NOT NULL
               AND id NOT IN (
                   SELECT MIN(id) FROM sms_messages
                   WHERE pdu IS NOT NULL
                   GROUP BY pdu
               )",
            [],
        ...
        conn.execute(
            "CREATE UNIQUE INDEX IF NOT EXISTS idx_sms_pdu_unique
             ON sms_messages(pdu)",
            [],
        )?;
        // Protocol-backed messages can be replayed by a modem or by the
        // VoWiFi helper's persisted receive history. Keep a durable tombstone
        // when the user deletes one so a later status sync cannot recreate it.
        conn.execute(
            "CREATE TABLE IF NOT EXISTS sms_deleted_markers (
                pdu TEXT PRIMARY KEY,
    ...
    /// Insert a protocol-backed SMS once, or refresh its delivery state when
    /// the same VoWiFi transaction is observed again.
    pub fn upsert_sms_by_pdu(
        &self,
        direction: &str,
        phone_number: &str,
        content: &str,
        ...
        let deleted = conn.query_row(
            "SELECT EXISTS(
                SELECT 1 FROM sms_deleted_markers WHERE pdu = ?1
            )",
            params![pdu],
        ...
        conn.execute(
            "INSERT INTO sms_messages (direction, phone_number, content, timestamp, status, pdu)
             ...
             WHERE sms_messages.direction IS NOT excluded.direction
                ...
                OR sms_messages.timestamp IS NOT excluded.timestamp
                OR sms_messages.status IS NOT excluded.status",
            params![direction, phone_number, content, timestamp, status, pdu],
        )?;
        conn.query_row(
            "SELECT id FROM sms_messages WHERE pdu = ?1 LIMIT 1",
            params![pdu],
            |row| row.get::<_, i64>(0),
        )
        .optional()
    ...
    pub fn clear_all_sms(&self) -> Result<()> {
        ...
        let tx = conn.transaction()?;
        tx.execute(
            "INSERT OR IGNORE INTO sms_deleted_markers (pdu)
             SELECT pdu FROM sms_messages
             WHERE pdu IS NOT NULL AND TRIM(pdu) <> ''",
            [],
        )?;
    ...
    pub fn delete_sms(&self, id: i64) -> Result<usize> {
        ...
        tx.execute(
            "INSERT OR IGNORE INTO sms_deleted_markers (pdu)
             SELECT pdu FROM sms_messages
             WHERE id = ?1 AND pdu IS NOT NULL AND TRIM(pdu) <> ''",
            params![id],
        )?;
        let deleted = tx.execute("DELETE FROM sms_messages WHERE id = ?1", params![id])?;
        tx.commit()?;
        Ok(deleted)
    }
    ...
    pub fn delete_sms_conversation(&self, phone_number: &str) -> Result<usize> {
        ...
        tx.execute(
            "INSERT OR IGNORE INTO sms_deleted_markers (pdu)
             SELECT pdu FROM sms_messages
             WHERE phone_number = ?1 AND pdu IS NOT NULL AND TRIM(pdu) <> ''",
            params![phone_number],
        )?;
...
mod tests {
    ...
    #[test]
    fn app_settings_round_trip_and_replace_values() {
        let db = test_db();
        assert_eq!(db.get_app_setting("app_config").unwrap(), None);

        db.set_app_setting("app_config", r#"{"data_enabled":false}"#)
            .unwrap();
        assert_eq!(
            db.get_app_setting("app_config").unwrap().as_deref(),
            Some(r#"{"data_enabled":false}"#)
        );

        db.set_app_setting("app_config", r#"{"data_enabled":true}"#)
            .unwrap();
        assert_eq!(
            db.get_app_setting("app_config").unwrap().as_deref(),
            Some(r#"{"data_enabled":true}"#)
        );
    }

    #[test]
    fn protocol_sms_upsert_is_idempotent_and_refreshes_state() {
        let db = test_db();
        let marker = "vowifi:rx:call-1:2a";
        let first_id = db
            .upsert_sms_by_pdu(
                "incoming",
                "+8615556250521",
        ...
        let messages = db.get_sms_messages(10, 0, Some("incoming")).unwrap();
    ...
    #[test]
    fn deleted_protocol_sms_is_not_recreated_by_history_replay() {
        let db = test_db();
        let marker = "vowifi:rx:deleted-call:2b";
        let id = db
            .upsert_sms_by_pdu(
                "incoming",
                "10086",
            ...
            .unwrap()
            .expect("first protocol observation should create a row");

        assert_eq!(db.delete_sms(id).unwrap(), 1);
        assert_eq!(
            ...
            .unwrap(),
            None,
            "persisted helper history must not recreate a user-deleted SMS"
        );
        assert!(db.get_sms_messages(10, 0, None).unwrap().is_empty());
    }

    #[test]
    fn protocol_sms_marker_has_sqlite_unique_invariant_across_connections() {
        let unique = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
            .as_nanos();
        let path = std::env::temp_dir().join(format!(
            "simadmin-sms-unique-{}-{unique}.db",
            std::process::id()
        ));
        let first = Database::new(path.clone()).unwrap();
        let second = Database::new(path.clone()).unwrap();
        first
            .upsert_sms_by_pdu(
                "incoming",
                "10086",
        ...
        let duplicate_error = second
            ...
            .execute(
                "INSERT INTO sms_messages
                 (direction, phone_number, content, timestamp, status, pdu)
                 VALUES ('incoming', '10086', 'duplicate', '2026-08-08 18:00:00', 'received', 'vowifi:rx:call:34')",
                [],
            )
            .unwrap_err();
        ...
        assert_eq!(
            first
                ...
                .len(),
            2,
            "different protocol markers must remain distinct even with identical content"
        );
    ...
    #[test]
    fn database_migration_collapses_legacy_duplicate_protocol_markers() {
        let unique = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
            .as_nanos();
        let path = std::env::temp_dir().join(format!(
            "simadmin-sms-migration-{}-{unique}.db",
            std::process::id()
        ));
        {
            ...
            legacy
                .execute_batch(
                    ...
                    INSERT INTO sms_messages
                        (direction, phone_number, content, timestamp, status, pdu)
                    VALUES
                        ('incoming', '10086', 'hello', '2026-08-08 18:00:00', 'received', 'vowifi:rx:legacy:34'),
                        ('incoming', '10086', 'hello', '2026-08-08 18:00:01', 'received', 'vowifi:rx:legacy:34');",
                )
                .unwrap();
        }

        let migrated = Database::new(path.clone()).unwrap();
        assert_eq!(
            migrated
                .get_sms_messages(10, 0, Some("incoming"))
                .unwrap()
