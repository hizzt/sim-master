...
use crate::modem_manager::{
    cache_smsc_for_identity, current_sim_identity, list_modem_paths, scope_request_modem,
};
...
async fn process_sms_path(
    ...
    if !incoming.smsc.is_empty() {
        if let Some(identity) =
            scope_request_modem(Some(modem_path.to_string()), current_sim_identity(conn)).await
        {
...
async fn scan_all_modems(
    ...
    match list_modem_paths(conn).await {
        ...
        Err(error) => {
            warn!(error = %error, reason = %reason, "Failed to enumerate modems for SMS scan");
        }
    }
}
...
///
/// 监听全部 ModemManager 模块的 Messaging.Added 信号。
pub async fn start_sms_listener(
    ...
    info!("Starting multi-modem SMS listener (ModemManager mode)");
    loop {
        let rule = format!(
            "type='signal',sender='{}',interface='{}',member='Added'",
            MM_SERVICE, MM_MESSAGING
        );
        if let Err(error) = call_dbus_match(&conn, "AddMatch", &rule).await {
            warn!(
                error = %error,
                retry_after_secs = MODEM_RETRY_DELAY_SECS,
                "Failed to register multi-modem SMS listener match"
            );
            tokio::time::sleep(Duration::from_secs(MODEM_RETRY_DELAY_SECS)).await;
            continue;
        }

        info!("SMS listener registered for all modem messaging interfaces");
        scan_all_modems(&conn, &db, &notification_sender, "initial", false).await;
            ...
            tokio::select! {
                maybe_msg = stream.next() => {
                    let msg = match maybe_msg {
                        Some(Ok(msg)) => msg,
                        Some(Err(error)) => {
                            warn!(error = %error, "SMS listener stream error");
                            break;
                        }
                        None => break,
                    };

                    if msg.header().member().is_none_or(|member| member.as_str() != "Added") {
                        continue;
                    }
                    let Some(modem_path) = msg.header().path().map(ToString::to_string) else {
                        continue;
                    };
                    let body = msg.body();
                    let Ok((sms_path, received)) = body
                    ...
                    info!(modem_path = %modem_path, path = %sms_path_str, "New SMS received");
                ...
                _ = poll_interval.tick() => {
                    scan_all_modems(&conn, &db, &notification_sender, "poll", true).await;
                }
        ...
        if let Err(error) = call_dbus_match(&conn, "RemoveMatch", &rule).await {
            warn!(error = %error, "Failed to remove SMS listener match");
        }
