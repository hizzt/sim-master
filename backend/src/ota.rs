...
const OTA_VOWIFI_HELPER_PATH: &str = "/opt/simadmin/simadmin-vowifi-helper";
const OTA_WWW_PATH: &str = "/opt/simadmin/www";
const OTA_META_PATH: &str = "/opt/simadmin/meta.json";
const OTA_THIRD_PARTY_NOTICES_PATH: &str = "/opt/simadmin/THIRD_PARTY_NOTICES.md";
...
const LATEST_RELEASE_API: &str = "https://api.github.com/repos/techblack/sim-master/releases/latest";
...
pub fn apply_ota_update(restart_now: bool) -> Result<String, String> {
    ...
    let staging_vowifi_helper = format!("{}/simadmin-vowifi-helper", OTA_STAGING_DIR);
    let staging_www = format!("{}/www", OTA_STAGING_DIR);

    install_binary_atomic(&staging_binary, OTA_BINARY_PATH)?;
    if Path::new(&staging_vowifi_helper).exists() {
        install_binary_atomic(&staging_vowifi_helper, OTA_VOWIFI_HELPER_PATH)?;
    }
...
fn install_third_party_notices() -> Result<(), String> {
    let staging_notices = format!("{}/THIRD_PARTY_NOTICES.md", OTA_STAGING_DIR);
    if !Path::new(&staging_notices).exists() {
        return Ok(());
    }
    fs::copy(&staging_notices, OTA_THIRD_PARTY_NOTICES_PATH)
        .map_err(|e| format!("Failed to copy third-party notices: {e}"))?;
    let output = Command::new("chmod")
        .args(["644", OTA_THIRD_PARTY_NOTICES_PATH])
        .output()
        .map_err(|e| format!("Failed to chmod third-party notices: {e}"))?;
    if !output.status.success() {
        return Err(format!(
            "Failed to chmod third-party notices: {}",
            String::from_utf8_lossy(&output.stderr)
        ));
    }
    Ok(())
...
fn fix_file_permissions() -> Result<(), String> {
    let binary_path = format!("{}/simadmin", OTA_STAGING_DIR);
    let vowifi_helper_path = format!("{}/simadmin-vowifi-helper", OTA_STAGING_DIR);
    let www_path = format!("{}/www", OTA_STAGING_DIR);
    ...
    if Path::new(&vowifi_helper_path).exists() {
        Command::new("chmod")
            .args(["755", &vowifi_helper_path])
            .output()
            .map_err(|e| format!("Failed to chmod VoWiFi helper: {e}"))?;
    }
