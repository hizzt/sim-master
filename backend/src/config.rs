...
fn default_ota_proxy_preset() -> String {
...
pub struct UiPreferences {
    ...
    #[serde(default = "default_true")]
    pub ota_proxy_enabled: bool,
    #[serde(default = "default_ota_proxy_preset")]
    pub ota_proxy_preset: String,
    #[serde(default)]
    pub ota_custom_proxy: String,
}
...
impl Default for UiPreferences {
    fn default() -> Self {
        Self {
            ...
            ota_proxy_enabled: default_true(),
            ota_proxy_preset: default_ota_proxy_preset(),
            ota_custom_proxy: String::new(),
        }
    }
}
...
pub struct UiPreferencesPatch {
    ...
    pub ota_proxy_enabled: Option<bool>,
    pub ota_proxy_preset: Option<String>,
    pub ota_custom_proxy: Option<String>,
}

#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
pub struct BandLockPreference {
    #[serde(default)]
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CellLockTarget {
    pub arfcn: u32,
    pub pci: u16,
}

#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
pub struct CellLockPreferences {
    #[serde(default)]
    pub lte: Option<CellLockTarget>,
    #[serde(default)]
    pub nr: Option<CellLockTarget>,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct OperatorSelectionPreference {
    pub mode: String,
    #[serde(default)]
    pub mccmnc: String,
}
...
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
pub struct ModemPreferences {
    #[serde(default)]
    pub airplane_mode: Option<bool>,
    #[serde(default)]
    pub radio_mode: Option<String>,
    #[serde(default)]
    pub band_lock: Option<BandLockPreference>,
    #[serde(default)]
    pub cell_lock: CellLockPreferences,
    #[serde(default)]
    pub operator_selection: Option<OperatorSelectionPreference>,
    #[serde(default)]
    pub voice_call_waiting: Option<bool>,
    #[serde(default)]
    pub ims_registration_requested: bool,
}
...
impl Default for AppConfig {
    fn default() -> Self {
        Self {
            ...
            modem: ModemPreferences::default(),
        }
    }
}

fn migrate_settings_schema(config: &mut AppConfig) -> bool {
    if config.settings_schema_version >= CURRENT_SETTINGS_SCHEMA_VERSION {
        return false;
    }

    if config.settings_schema_version < 1 {
        // Schema v0 predates persisted airplane mode. Its false value was only
        // a deserialization default, not an explicit user preference.
        config.modem.airplane_mode = None;
    }
    config.settings_schema_version = CURRENT_SETTINGS_SCHEMA_VERSION;
    true
}
...
impl ConfigManager {
    /// 创建新的配置管理器
    pub fn new(database: Arc<Database>, legacy_config_path: PathBuf) -> Result<Self, String> {
        let stored = database
            .get_app_setting(APP_CONFIG_SETTING_KEY)
            .map_err(|e| format!("Failed to read config from SQLite: {e}"))?;
        let (mut config, needs_save) = match stored {
            Some(content) => (
                serde_json::from_str::<AppConfig>(&content)
                    .map_err(|e| format!("Failed to parse config stored in SQLite: {e}"))?,
                false,
            ),
            None if legacy_config_path.exists() => {
                let content = fs::read_to_string(&legacy_config_path).map_err(|e| {
                    format!(
                        "Failed to read legacy config {}: {e}",
                        legacy_config_path.display()
                    )
                })?;
                let config = serde_json::from_str::<AppConfig>(&content).map_err(|e| {
                    format!(
                        "Failed to parse legacy config {}: {e}",
                        legacy_config_path.display()
                    )
                })?;
                info!(path = %legacy_config_path.display(), "Migrating config file to SQLite");
                (config, true)
            }
            None => {
        ...
        let template_changed = migrate_update_templates(&mut config);
        let schema_changed = migrate_settings_schema(&mut config);
        let changed = template_changed || schema_changed;
    ...
    pub fn set_automation_config(&self, automation: AutomationConfig) -> Result<(), String> {
        self.update(|config| config.automation = automation)
    }
    ...
    pub fn set_backup_config(&self, backup: BackupConfig) -> Result<(), String> {
        self.update(|config| config.backup = backup)
    }
    ...
    pub fn set_esim_config(&self, esim: EsimConfig) -> Result<(), String> {
        self.update(|config| config.esim = esim)
    }
    ...
    pub fn get_wlan_enabled(&self) -> Option<bool> {
        self.config.read().unwrap().device_network.wlan_enabled
    }
    ...
    pub fn get_modem_preferences(&self) -> ModemPreferences {
        self.config.read().unwrap().modem.clone()
    }
    ...
    pub fn set_security(&self, security: SecurityConfig) -> Result<(), String> {
        self.update(|config| config.security = security)
    }

    pub fn set_data_enabled(&self, enabled: bool) -> Result<(), String> {
        self.update(|config| config.data_enabled = enabled)
    }

    pub fn set_apn_config(&self, apn: ApnConfig) -> Result<(), String> {
        self.update(|config| config.apn = apn)
    }

    pub fn set_work_mode(&self, mode: WorkMode) -> Result<(), String> {
        self.update(|config| config.work_mode = mode)
    }

    pub fn set_roaming_allowed(&self, allowed: bool) -> Result<(), String> {
        self.update(|config| config.roaming_allowed = allowed)
    }

    pub fn set_ddns_config(&self, ddns: DdnsConfig) -> Result<(), String> {
        self.update(|config| config.device_network.ddns = ddns)
    }

    pub fn set_wlan_enabled(&self, enabled: bool) -> Result<(), String> {
        self.update(|config| config.device_network.wlan_enabled = Some(enabled))
    }

    pub fn set_ui_preferences(&self, ui: UiPreferences) -> Result<(), String> {
        self.update(|config| config.ui = ui)
    }

    pub fn patch_ui_preferences(&self, patch: UiPreferencesPatch) -> Result<UiPreferences, String> {
        let mut current = self.config.write().unwrap();
        let mut next = current.clone();
        if let Some(value) = patch.theme_mode {
            next.ui.theme_mode = value;
        }
        if let Some(value) = patch.refresh_interval_ms {
            next.ui.refresh_interval_ms = value;
        }
        if let Some(value) = patch.ota_proxy_enabled {
            next.ui.ota_proxy_enabled = value;
        }
        if let Some(value) = patch.ota_proxy_preset {
            next.ui.ota_proxy_preset = value;
        }
        if let Some(value) = patch.ota_custom_proxy {
            next.ui.ota_custom_proxy = value;
        }
        self.persist(&next)?;
        let preferences = next.ui.clone();
        *current = next;
        Ok(preferences)
    }

    pub fn set_airplane_mode(&self, enabled: bool) -> Result<(), String> {
        self.update(|config| config.modem.airplane_mode = Some(enabled))
    }

    pub fn set_radio_mode(&self, mode: String) -> Result<(), String> {
        self.update(|config| config.modem.radio_mode = Some(mode))
    }

    pub fn set_band_lock(&self, band_lock: BandLockPreference) -> Result<(), String> {
        self.update(|config| config.modem.band_lock = Some(band_lock))
    }

    pub fn set_cell_lock(&self, cell_lock: CellLockPreferences) -> Result<(), String> {
        self.update(|config| config.modem.cell_lock = cell_lock)
    }

    pub fn set_operator_selection(
        &self,
        operator_selection: OperatorSelectionPreference,
    ) -> Result<(), String> {
        self.update(|config| config.modem.operator_selection = Some(operator_selection))
    }

    pub fn set_voice_call_waiting(&self, enabled: bool) -> Result<(), String> {
        self.update(|config| config.modem.voice_call_waiting = Some(enabled))
    }

    pub fn set_ims_registration_requested(&self, requested: bool) -> Result<(), String> {
        self.update(|config| config.modem.ims_registration_requested = requested)
    }

    pub fn set_last_notified_update_version(&self, version: String) -> Result<(), String> {
        self.update(|config| {
            config.version_update_notifications.last_notified_version = Some(version)
        })
    }
    ...
    fn update<F>(&self, updater: F) -> Result<(), String>
    where
    ...
    fn persist(&self, config: &AppConfig) -> Result<(), String> {
        ...
        self.database
            .set_app_setting(APP_CONFIG_SETTING_KEY, &content)
            .map_err(|e| format!("Failed to save config to SQLite: {e}"))
    }
}
...
mod persistence_tests {
    ...
    fn missing_legacy_path() -> PathBuf {
        std::env::temp_dir().join(format!(
            "simadmin-config-does-not-exist-{}.json",
            std::process::id()
        ))
    }

    #[test]
    fn config_reloads_all_runtime_preferences_from_sqlite() {
        let database = Arc::new(Database::new(PathBuf::from(":memory:")).unwrap());
        let manager = ConfigManager::new(Arc::clone(&database), missing_legacy_path()).unwrap();

        manager
            .set_ui_preferences(UiPreferences {
                theme_mode: "dark".to_string(),
                refresh_interval_ms: 10_000,
                ota_proxy_enabled: false,
                ota_proxy_preset: "custom".to_string(),
                ota_custom_proxy: "https://proxy.example/".to_string(),
            })
            .unwrap();
        manager.set_airplane_mode(true).unwrap();
        manager.set_radio_mode("lte".to_string()).unwrap();
        manager
            .set_band_lock(BandLockPreference {
                lte_fdd_bands: vec![1, 3],
                ..BandLockPreference::default()
            })
            .unwrap();
        manager
            .set_cell_lock(CellLockPreferences {
                lte: Some(CellLockTarget {
                    arfcn: 1300,
                    pci: 7,
                }),
                nr: None,
            ...
            .unwrap();
        manager.set_wlan_enabled(false).unwrap();
        manager
            .set_operator_selection(OperatorSelectionPreference {
                mode: "manual".to_string(),
                mccmnc: "46000".to_string(),
            })
            .unwrap();
        manager.set_voice_call_waiting(true).unwrap();
        manager.set_ims_registration_requested(true).unwrap();

        let reloaded = ConfigManager::new(database, missing_legacy_path()).unwrap();
        assert_eq!(
            reloaded.get_ui_preferences(),
            UiPreferences {
                theme_mode: "dark".to_string(),
                refresh_interval_ms: 10_000,
                ota_proxy_enabled: false,
        ...
        assert_eq!(reloaded.get_wlan_enabled(), Some(false));
        assert_eq!(
            reloaded.get_modem_preferences(),
            ModemPreferences {
                airplane_mode: Some(true),
                radio_mode: Some("lte".to_string()),
                band_lock: Some(BandLockPreference {
                    lte_fdd_bands: vec![1, 3],
                    ..BandLockPreference::default()
                }),
                cell_lock: CellLockPreferences {
                    lte: Some(CellLockTarget {
                        arfcn: 1300,
                        pci: 7
                    }),
                ...
                operator_selection: Some(OperatorSelectionPreference {
                    mode: "manual".to_string(),
                    mccmnc: "46000".to_string(),
                }),
                voice_call_waiting: Some(true),
                ims_registration_requested: true,
            }
        );
    }

    #[test]
    fn legacy_json_is_migrated_once_into_sqlite() {
        let database = Arc::new(Database::new(PathBuf::from(":memory:")).unwrap());
        let legacy_path = std::env::temp_dir().join(format!(
            "simadmin-legacy-config-{}.json",
            std::process::id()
        ));
        let mut legacy = AppConfig::default();
        legacy.data_enabled = true;
        legacy.ui.theme_mode = "dark".to_string();
        fs::write(&legacy_path, serde_json::to_string(&legacy).unwrap()).unwrap();

        let manager = ConfigManager::new(Arc::clone(&database), legacy_path.clone()).unwrap();
        assert!(manager.get_data_enabled());
        assert_eq!(manager.get_ui_preferences().theme_mode, "dark");

        legacy.data_enabled = false;
        fs::write(&legacy_path, serde_json::to_string(&legacy).unwrap()).unwrap();
        let reloaded = ConfigManager::new(database, legacy_path.clone()).unwrap();
        assert!(reloaded.get_data_enabled());

        let _ = fs::remove_file(legacy_path);
    }

    #[test]
    fn schema_v0_default_airplane_mode_is_not_treated_as_user_choice() {
        let database = Arc::new(Database::new(PathBuf::from(":memory:")).unwrap());
        database
            .set_app_setting(
                APP_CONFIG_SETTING_KEY,
                r#"{"modem":{"airplane_mode":false}}"#,
            )
            .unwrap();

        let manager = ConfigManager::new(database, missing_legacy_path()).unwrap();
        assert_eq!(
            manager.get_config().settings_schema_version,
            CURRENT_SETTINGS_SCHEMA_VERSION
        );
        assert_eq!(manager.get_modem_preferences().airplane_mode, None);
    }

    #[test]
    fn schema_v1_explicit_airplane_mode_survives_ui_preferences_migration() {
        let database = Arc::new(Database::new(PathBuf::from(":memory:")).unwrap());
        database
            .set_app_setting(
                APP_CONFIG_SETTING_KEY,
                r#"{"settings_schema_version":1,"modem":{"airplane_mode":false}}"#,
            )
            .unwrap();
        ...
        assert_eq!(manager.get_modem_preferences().airplane_mode, Some(false));
        assert_eq!(manager.get_ui_preferences(), UiPreferences::default());
    }
}
