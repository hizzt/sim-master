//! 小区锁定状态（无底层锁网能力时由 SQLite 配置恢复展示）。
use crate::config::{CellLockPreferences, CellLockTarget};
...
impl CellLockStore {
    pub fn from_preferences(preferences: &CellLockPreferences) -> Self {
        Self {
            lte: preferences.lte.as_ref().map(|item| (item.arfcn, item.pci)),
            nr: preferences.nr.as_ref().map(|item| (item.arfcn, item.pci)),
        }
    }

    pub fn preferences(&self) -> CellLockPreferences {
        CellLockPreferences {
            lte: self.lte.map(|(arfcn, pci)| CellLockTarget { arfcn, pci }),
            nr: self.nr.map(|(arfcn, pci)| CellLockTarget { arfcn, pci }),
        }
    }
