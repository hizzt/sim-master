...
use handlers::*;
use models::{BandLockRequest, RadioMode, WlanEnabledRequest};
use modem_manager::{
    ensure_nm_modem_profile, init_data_connection, register_operator_auto,
    register_operator_manual, set_airplane_mode, set_band_lock, set_call_waiting, set_radio_mode,
    start_ims_registration,
};
...
use vowifi_tunnel::VowifiTunnelManager;
...
fn get_proxy_config_path() -> PathBuf {
    std::env::current_exe()
        .expect("Failed to get executable path")
        .parent()
        .expect("Failed to get executable directory")
...
async fn restore_persisted_device_settings(
    dbus_conn: Arc<Connection>,
    config_manager: Arc<ConfigManager>,
) {
    let modem = config_manager.get_modem_preferences();
    let airplane_enabled = modem.airplane_mode.unwrap_or(false);
    let mut airplane_pending = modem.airplane_mode;
    let mut radio_pending = (!airplane_enabled)
        .then(|| modem.radio_mode.as_deref())
        .flatten()
        .and_then(|value| match value {
            "auto" => Some(RadioMode::Auto),
            "lte" => Some(RadioMode::LteOnly),
            "nr" => Some(RadioMode::NrOnly),
            _ => None,
        });
    let mut bands_pending = (!airplane_enabled).then_some(modem.band_lock).flatten();
    let mut operator_pending = (!airplane_enabled)
        .then_some(modem.operator_selection)
        .flatten();
    let mut call_waiting_pending = (!airplane_enabled)
        .then_some(modem.voice_call_waiting)
        .flatten();
    let mut ims_pending = !airplane_enabled && modem.ims_registration_requested;
    let mut wlan_pending = config_manager.get_wlan_enabled();

    for attempt in 1..=6 {
        if let Some(enabled) = airplane_pending {
            match set_airplane_mode(&dbus_conn, enabled).await {
                Ok(()) => airplane_pending = None,
                Err(error) => warn!(attempt, error = %error, "Failed to restore airplane mode"),
            }
        }

        if airplane_pending.is_none() && !airplane_enabled {
            if let Some(mode) = radio_pending.clone() {
                match set_radio_mode(&dbus_conn, mode).await {
                    Ok(()) => radio_pending = None,
                    Err(error) => warn!(attempt, error = %error, "Failed to restore radio mode"),
                }
            }

            if let Some(bands) = bands_pending.as_ref() {
                let request = BandLockRequest {
                    lte_fdd_bands: bands.lte_fdd_bands.clone(),
                    lte_tdd_bands: bands.lte_tdd_bands.clone(),
                    nr_fdd_bands: bands.nr_fdd_bands.clone(),
                    nr_tdd_bands: bands.nr_tdd_bands.clone(),
                };
                match set_band_lock(&dbus_conn, &request).await {
                    Ok(()) => bands_pending = None,
                    Err(error) => warn!(attempt, error = %error, "Failed to restore band lock"),
                }
            }

            if let Some(operator) = operator_pending.as_ref() {
                let result = match operator.mode.as_str() {
                    "auto" => register_operator_auto(&dbus_conn).await,
                    "manual" if matches!(operator.mccmnc.len(), 5 | 6) => {
                        register_operator_manual(&dbus_conn, &operator.mccmnc).await
                    }
                    _ => {
                        warn!(mode = %operator.mode, "Ignoring invalid saved operator selection");
                        operator_pending = None;
                        Ok(())
                    }
                };
                match result {
                    Ok(()) => operator_pending = None,
                    Err(error) => {
                        warn!(attempt, error = %error, "Failed to restore operator selection")
                    }
                }
            }

            if let Some(enabled) = call_waiting_pending {
                match set_call_waiting(&dbus_conn, enabled).await {
                    Ok(()) => call_waiting_pending = None,
                    Err(error) => {
                        warn!(attempt, error = %error, "Failed to restore call waiting setting")
                    }
                }
            }

            if ims_pending {
                match start_ims_registration(&dbus_conn).await {
                    Ok(_) => ims_pending = false,
                    Err(error) => {
                        warn!(attempt, error = %error, "Failed to restore IMS registration request")
                    }
                }
            }
        }

        if let Some(enabled) = wlan_pending {
            match crate::device_network::wlan_set_enabled(WlanEnabledRequest { enabled }).await {
                Ok(_) => wlan_pending = None,
                Err(error) => warn!(attempt, error = %error, "Failed to restore WLAN setting"),
            }
        }

        if airplane_pending.is_none()
            && radio_pending.is_none()
            && bands_pending.is_none()
            && operator_pending.is_none()
            && call_waiting_pending.is_none()
            && !ims_pending
            && wlan_pending.is_none()
        {
            info!(attempt, "Persisted device settings restored");
            return;
        }
    ...
    warn!("Some persisted device settings could not be restored after retries");
}
...
async fn modem_context_middleware(
    request: axum::extract::Request,
    next: middleware::Next,
) -> Response {
    let modem_id = request
        .headers()
        .get("x-simadmin-modem-id")
        .and_then(|value| value.to_str().ok())
        .map(str::trim)
        .filter(|value| !value.is_empty() && value.len() <= 256)
        .map(str::to_string);
    modem_manager::scope_request_modem(modem_id, next.run(request)).await
}
...
async fn main() -> Result<()> {
    ...
    if let Some(CliCommand::Auth { command }) = &cli.command {
        ...
        let config_manager = ConfigManager::new(Arc::clone(&db), get_default_config_path())
    ...
    let proxy_manager =
        Arc::new(ProxyManager::load(get_proxy_config_path()).map_err(anyhow::Error::msg)?);
    // 初始化配置管理器
    let config_path = get_default_config_path();
    info!(database = ?db_path, legacy_path = ?config_path, "Loading config");
    let config_manager =
        Arc::new(ConfigManager::new(Arc::clone(&app_db), config_path).map_err(anyhow::Error::msg)?);
    let data_user_disabled = Arc::new(AtomicBool::new(!config_manager.get_data_enabled()));
    let airplane_mode_requested = Arc::new(AtomicBool::new(
        config_manager
            .get_modem_preferences()
            .airplane_mode
            .unwrap_or(false),
    ));
    let vowifi_tunnel_manager = Arc::new(VowifiTunnelManager::new_default(
        Arc::clone(&app_db),
        Arc::clone(&dbus_conn),
        Arc::clone(&airplane_mode_requested),
    ));
    ...
    if let Some(desired_airplane_mode) = config_manager.get_modem_preferences().airplane_mode {
        match set_airplane_mode(&dbus_conn, desired_airplane_mode).await {
            Ok(()) => info!(
                enabled = desired_airplane_mode,
                "Restored airplane mode setting"
            ),
            Err(error) => warn!(
                enabled = desired_airplane_mode,
                error = %error,
                "Failed to restore airplane mode setting"
            ),
        }
    }

    {
        ...
        tokio::spawn(async move {
            ...
            restore_persisted_device_settings(conn, config).await;
        });
    }
    ...
    // 自动初始化数据连接
    {
        ...
        tokio::spawn(async move {
            // 等待 2 秒让 modem 完全初始化
            tokio::time::sleep(tokio::time::Duration::from_secs(2)).await;
            if cfg.get_modem_preferences().airplane_mode == Some(true) {
                tracing::info!("Auto-connect skipped because airplane mode is enabled");
                return;
            }
    ...
    let app_state = AppState::new(
        ...
        Arc::clone(&vowifi_tunnel_manager),
        sms_resync,
        data_user_disabled,
    ...
    {
        let manager = Arc::clone(&proxy_manager);
        let conn = Arc::clone(&app_state.dbus_conn);
        tokio::spawn(async move {
            tokio::time::sleep(tokio::time::Duration::from_secs(3)).await;
            manager.start_enabled(&conn).await;
        });
    }
    ...
    let protected_routes = Router::new()
        .route(
            "/api/ui/preferences/save",
            post(set_ui_preferences_handler).options(options_handler),
        )
        // ========== 设备信息接口 ==========
        .route(
            "/api/modems",
            get(get_modems_handler).options(options_handler),
        )
        ...
        .route(
            ...
            axum::routing::put(update_proxy_handler)
                .delete(delete_proxy_handler)
                .options(options_handler),
        )
        .route(
            "/api/proxies/{id}/start",
            post(start_proxy_handler).options(options_handler),
        )
        .route(
            "/api/proxies/{id}/stop",
            post(stop_proxy_handler).options(options_handler),
        )
        .route(
        ...
        .route(
            "/api/ims/register",
            post(start_ims_registration_handler).options(options_handler),
        )
        .route(
            "/api/vowifi/tunnel/status",
            get(get_vowifi_tunnel_status_handler).options(options_handler),
        )
        .route(
            "/api/vowifi/verification/status",
            get(get_vowifi_verification_status_handler).options(options_handler),
        )
        .route(
            "/api/vowifi/sms/verify",
            post(verify_vowifi_sms_path_handler).options(options_handler),
        )
        .route(
            "/api/vowifi/tunnel/start",
            post(start_vowifi_tunnel_handler).options(options_handler),
        )
        .route(
            "/api/vowifi/tunnel/stop",
            post(stop_vowifi_tunnel_handler).options(options_handler),
        )
        .route(
            "/api/vowifi/call/dial",
            post(dial_vowifi_call_handler).options(options_handler),
        )
        .route(
            "/api/vowifi/call/hangup",
            post(hangup_vowifi_call_handler).options(options_handler),
        )
        .route(
            "/api/vowifi/call/status",
            get(get_vowifi_call_status_handler).options(options_handler),
        )
        .route(
            "/api/vowifi/call/audio/play",
            post(play_vowifi_call_audio_handler)
                .options(options_handler)
                .layer(DefaultBodyLimit::max(4 * 1024 * 1024)),
        )
        .route(
            "/api/vowifi/call/audio/recording",
            post(get_vowifi_call_recording_handler).options(options_handler),
        )
        .route(
            "/api/vowifi/call/audio/stats",
            post(get_vowifi_call_audio_stats_handler).options(options_handler),
        )
        .route(
        ...
        .layer(middleware::from_fn(modem_context_middleware));

    let shutdown_vowifi_tunnel_manager = Arc::clone(&vowifi_tunnel_manager);
    let app = Router::new()
        .route("/api/health", get(health_check).options(options_handler))
        .route(
            "/api/ui/preferences",
            get(get_ui_preferences_handler).options(options_handler),
        )
        .route(
            "/api/auth/status",
    ...
    axum::serve(listener, app)
        .with_graceful_shutdown(async move {
            ...
            if let Err(error) = shutdown_vowifi_tunnel_manager.stop().await {
                warn!(error = %error, "Failed to stop VoWiFi SWu tunnel during shutdown");
            }
        })
        .await?;
