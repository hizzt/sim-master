...
use crate::{
    config::{
        ApnConfig, BandLockPreference, OperatorSelectionPreference, UiPreferences,
        UiPreferencesPatch,
    },
    esim::EsimApiError,
    models::*,
    modem_manager::{
        ...
        get_cells_data, get_data_connection_status, get_device_info_data, get_ims_status_data,
        get_is_roaming_mm, get_modems_data, get_network_info_data, get_operators_list,
        ...
        power_cycle_sim_for_profile_switch, prepare_vowifi_tunnel_config,
        ...
        sim_details_cache_missing, start_cell_monitoring, start_ims_registration,
        stop_cell_monitoring,
    },
    proxy::{ProxyStatus, ProxyUpsertRequest},
    state::AppState,
    system_event::{
    ...
    vowifi_tunnel::{
        SmsPathVerificationRequest, SmsPathVerificationResult, VowifiCallAudio,
        VowifiCallAudioPlayRequest, VowifiCallAudioRequest, VowifiCallStatus,
        VowifiDialCallRequest, VowifiHangupCallRequest, VowifiTunnelStartRequest,
        VowifiTunnelStatus,
    },
};
...
pub async fn get_ui_preferences_handler(State(app): State<AppState>) -> impl IntoResponse {
    (
        StatusCode::OK,
        Json(ApiResponse::success_with_message(
            "Success",
            app.config_manager.get_ui_preferences(),
        )),
    )
}

/// POST /api/ui/preferences/save
pub async fn set_ui_preferences_handler(
        ...
        .is_some_and(|value| !matches!(value, "light" | "dark"))
    {
        return (
            ...
            Json(ApiResponse::<UiPreferences>::error(
    ...
    if patch
        .refresh_interval_ms
        .is_some_and(|value| !matches!(value, 0 | 1_000 | 3_000 | 5_000 | 10_000))
    {
        return (
            StatusCode::BAD_REQUEST,
            Json(ApiResponse::<UiPreferences>::error(
                "refresh_interval_ms is not supported",
            )),
        );
    }
    if patch.ota_proxy_preset.as_deref().is_some_and(|value| {
        !matches!(
            value,
            "https://gh-proxy.com/" | "https://ghproxy.net/" | "https://githubproxy.cc/" | "custom"
        )
    }) {
        return (
            StatusCode::BAD_REQUEST,
            Json(ApiResponse::<UiPreferences>::error(
                "ota_proxy_preset is not supported",
            )),
        );
    }
    if patch
        ...
        .is_some_and(|value| value.len() > 2_048)
    {
        return (
            StatusCode::BAD_REQUEST,
            Json(ApiResponse::<UiPreferences>::error(
                "ota_custom_proxy is too long",
            )),
        );
    }
    match app.config_manager.patch_ui_preferences(patch) {
        Ok(preferences) => (
            StatusCode::OK,
            Json(ApiResponse::success_with_message("Saved", preferences)),
        ),
        Err(error) => (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(ApiResponse::<UiPreferences>::error(error)),
        ),
    }
}
...
/// GET /api/modems
pub async fn get_modems_handler(State(conn): State<Arc<Connection>>) -> impl IntoResponse {
    match get_modems_data(&conn).await {
        Ok(data) => (
            StatusCode::OK,
            Json(ApiResponse::success_with_message("Success", data)),
        ),
        Err(error) => (
            StatusCode::OK,
            Json(ApiResponse::<ModemListResponse>::error(format!(
                "Failed to list modems: {error}"
            ))),
        ),
    }
}
...
pub async fn list_proxies_handler(State(app): State<AppState>) -> impl IntoResponse {
    (
        StatusCode::OK,
        Json(ApiResponse::success_with_message(
            "Success",
            app.proxy_manager.list().await,
        )),
    )
}
...
pub async fn create_proxy_handler(
    ...
    match app.proxy_manager.create(&app.dbus_conn, payload).await {
        Ok(status) => (
            StatusCode::CREATED,
            Json(ApiResponse::success_with_message("Proxy created", status)),
        ),
        Err(error) => (
            StatusCode::BAD_REQUEST,
            Json(ApiResponse::<ProxyStatus>::error(error)),
        ),
    }
}
...
pub async fn update_proxy_handler(
    ...
    match app.proxy_manager.update(&app.dbus_conn, &id, payload).await {
        Ok(status) => (
            StatusCode::OK,
            Json(ApiResponse::success_with_message("Proxy updated", status)),
        ),
        Err(error) => (
            StatusCode::BAD_REQUEST,
            Json(ApiResponse::<ProxyStatus>::error(error)),
...
pub async fn delete_proxy_handler(
    ...
    match app.proxy_manager.delete(&id).await {
        Ok(()) => (
            StatusCode::OK,
            Json(ApiResponse::success_with_message(
                "Proxy deleted",
...
pub async fn start_proxy_handler(
    State(app): State<AppState>,
    Path(id): Path<String>,
) -> impl IntoResponse {
    match app.proxy_manager.start(&app.dbus_conn, &id).await {
        Ok(status) => (
            StatusCode::OK,
            Json(ApiResponse::success_with_message("Proxy started", status)),
        ),
        Err(error) => (
            StatusCode::BAD_REQUEST,
            Json(ApiResponse::<ProxyStatus>::error(error)),
...
pub async fn stop_proxy_handler(
    State(app): State<AppState>,
    Path(id): Path<String>,
) -> impl IntoResponse {
    match app.proxy_manager.stop(&id).await {
        Ok(status) => (
            StatusCode::OK,
            Json(ApiResponse::success_with_message("Proxy stopped", status)),
        ),
        Err(error) => (
            StatusCode::BAD_REQUEST,
            Json(ApiResponse::<ProxyStatus>::error(error)),
...
pub async fn set_radio_mode_handler(
    ...
    let persisted_mode = match &payload.mode {
        RadioMode::Auto => "auto",
        RadioMode::LteOnly => "lte",
        RadioMode::NrOnly => "nr",
    };
    match set_radio_mode(&app.dbus_conn, payload.mode).await {
        Ok(()) => match app
            .config_manager
            .set_radio_mode(persisted_mode.to_string())
        {
            Ok(()) => (
                StatusCode::OK,
                Json(ApiResponse::success_with_message(
...
/// GET /api/band-lock
pub async fn get_band_lock_handler(State(app): State<AppState>) -> impl IntoResponse {
    match get_band_lock_status(&app.dbus_conn).await {
        Ok(mut data) => {
            data.saved_config = app
                .config_manager
                .get_modem_preferences()
                .band_lock
                .map(|saved| BandLockRequest {
                    lte_fdd_bands: saved.lte_fdd_bands,
                    lte_tdd_bands: saved.lte_tdd_bands,
                    nr_fdd_bands: saved.nr_fdd_bands,
                    nr_tdd_bands: saved.nr_tdd_bands,
                });
            (
                StatusCode::OK,
                Json(ApiResponse::success_with_message("Success", data)),
...
pub async fn set_band_lock_handler(
    ...
    match set_band_lock(&app.dbus_conn, &payload).await {
        Ok(()) => {
            let saved = BandLockPreference {
            ...
            match app.config_manager.set_band_lock(saved) {
                Ok(()) => (
                    StatusCode::OK,
                    Json(ApiResponse::success_with_message(
...
pub async fn register_network_manual(
    ...
    let mccmnc = payload.mccmnc.trim();
    if !matches!(mccmnc.len(), 5 | 6) || !mccmnc.bytes().all(|byte| byte.is_ascii_digit()) {
        return (
            StatusCode::BAD_REQUEST,
            Json(ApiResponse::<serde_json::Value>::error(
                "MCCMNC must contain 5 or 6 digits",
            )),
        );
    }
    match register_operator_manual(&app.dbus_conn, mccmnc).await {
        Ok(()) => match app
            .config_manager
            .set_operator_selection(OperatorSelectionPreference {
...
pub async fn register_network_auto(State(app): State<AppState>) -> impl IntoResponse {
    match register_operator_auto(&app.dbus_conn).await {
        Ok(()) => match app
            .config_manager
            .set_operator_selection(OperatorSelectionPreference {
                mode: "auto".to_string(),
...
pub async fn set_cell_lock_handler(
    ...
    match store.apply(&payload) {
        Ok(()) => match app.config_manager.set_cell_lock(store.preferences()) {
            Ok(()) => (
                StatusCode::OK,
                Json(ApiResponse::success_with_message(
                    "OK",
            ...
            Err(error) => {
                *store = previous;
                (
                    StatusCode::INTERNAL_SERVER_ERROR,
                    Json(ApiResponse::<CellLockResult>::error(error)),
                )
            }
        },
        Err(e) => (
...
pub async fn unlock_all_cells_handler(State(app): State<AppState>) -> impl IntoResponse {
    ...
    let previous = store.clone();
    store.unlock_all();
    match app.config_manager.set_cell_lock(store.preferences()) {
        Ok(()) => (
            StatusCode::OK,
            Json(ApiResponse::success_with_message(
                "Unlocked",
...
pub async fn set_device_wlan_enabled_handler(
    ...
    match crate::device_network::wlan_set_enabled(payload).await {
        Ok(data) => match app.config_manager.set_wlan_enabled(enabled) {
            Ok(()) => (
                StatusCode::OK,
                Json(ApiResponse::success_with_message(
                    "WLAN state updated",
            ...
            Err(error) => (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(ApiResponse::<WlanStatusResponse>::error(error)),
            ),
        },
        Err(err) => (
            StatusCode::OK,
...
pub async fn set_airplane_mode_handler(
    ...
    match set_airplane_mode(&app.dbus_conn, payload.enabled).await {
        Ok(_) => {
            ...
            if let Err(error) = app.config_manager.set_airplane_mode(payload.enabled) {
                return (
                    StatusCode::INTERNAL_SERVER_ERROR,
                    Json(ApiResponse::<AirplaneModeResponse>::error(error)),
                );
            }
...
pub async fn set_call_settings_handler(
    ...
    match set_call_waiting(&app.dbus_conn, enabled).await {
        Ok(()) => match app.config_manager.set_voice_call_waiting(enabled) {
            Ok(()) => (
                StatusCode::OK,
                Json(ApiResponse::success_with_message(
...
pub async fn get_ims_status_handler(State(conn): State<Arc<Connection>>) -> impl IntoResponse {
    match get_ims_status_data(&conn).await {
        Ok(status) => (
            StatusCode::OK,
            Json(ApiResponse::success_with_message(
                "IMS status retrieved",
                status,
            )),
        ),
        Err(error) => (
            StatusCode::OK,
            Json(ApiResponse::<ImsStatusResponse>::error(format!(
                "Failed to read IMS status: {error}"
            ))),
        ),
    }
}

pub async fn start_ims_registration_handler(State(app): State<AppState>) -> impl IntoResponse {
    match start_ims_registration(&app.dbus_conn).await {
        Ok(status) => match app.config_manager.set_ims_registration_requested(true) {
            Ok(()) => (
                StatusCode::OK,
                Json(ApiResponse::success_with_message(
                    "IMS registration requested",
                    status,
                )),
            ),
            Err(error) => (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(ApiResponse::<ImsStatusResponse>::error(error)),
            ),
        },
        Err(error) => (
            StatusCode::OK,
            Json(ApiResponse::<ImsStatusResponse>::error(format!(
                "Failed to request IMS registration: {error}"
            ))),
        ),
    }
}

pub async fn get_vowifi_tunnel_status_handler(State(app): State<AppState>) -> impl IntoResponse {
    let status = app.vowifi_tunnel_manager.status().await;
    (
        StatusCode::OK,
        Json(ApiResponse::success_with_message(
            "VoWiFi SWu tunnel status retrieved",
            status,
        )),
    )
}

pub async fn get_vowifi_verification_status_handler(
    State(app): State<AppState>,
) -> impl IntoResponse {
    let status = app.vowifi_tunnel_manager.verification_status().await;
    (
        StatusCode::OK,
        Json(ApiResponse::success_with_message(
            "VoWiFi verification status retrieved",
            status,
        )),
    )
}

pub async fn verify_vowifi_sms_path_handler(
    State(app): State<AppState>,
    Json(payload): Json<SmsPathVerificationRequest>,
) -> (StatusCode, Json<ApiResponse<SmsPathVerificationResult>>) {
    match app.vowifi_tunnel_manager.verify_sms_path(&payload).await {
        Ok(result) => (
            StatusCode::OK,
            Json(ApiResponse::success_with_message(
                "VoWiFi SMS path verification completed",
                result,
            )),
        ),
        Err(error) => (
            StatusCode::BAD_REQUEST,
            Json(ApiResponse::<SmsPathVerificationResult>::error(error)),
        ),
    }
}

pub async fn start_vowifi_tunnel_handler(
    State(app): State<AppState>,
    payload: Option<Json<VowifiTunnelStartRequest>>,
) -> impl IntoResponse {
    let config = match prepare_vowifi_tunnel_config(&app.dbus_conn, &app.database).await {
        Ok(config) => config,
        Err(error) => {
            return (
                StatusCode::BAD_REQUEST,
                Json(ApiResponse::<VowifiTunnelStatus>::error(error)),
            )
        }
    };
    match app
        .vowifi_tunnel_manager
        .start(config, payload.and_then(|Json(payload)| payload.proxy))
        .await
    {
        Ok(status) => (
            StatusCode::ACCEPTED,
            Json(ApiResponse::success_with_message(
                "VoWiFi SWu tunnel start requested",
                status,
            )),
        ),
        Err(error) => (
...
pub async fn stop_vowifi_tunnel_handler(State(app): State<AppState>) -> impl IntoResponse {
    match app.vowifi_tunnel_manager.stop().await {
        Ok(status) => (
            StatusCode::OK,
            Json(ApiResponse::success_with_message(
                "VoWiFi SWu tunnel stopped",
                status,
            )),
        ),
        Err(error) => (
...
/// allocates RTP/RTCP transport and attaches file-backed audio when G.711 is
/// negotiated. Live microphone and speaker devices are not attached.
pub async fn dial_vowifi_call_handler(
    State(app): State<AppState>,
...
/// POST /api/vowifi/call/hangup
pub async fn hangup_vowifi_call_handler(
    State(app): State<AppState>,
...
/// GET /api/vowifi/call/status
pub async fn get_vowifi_call_status_handler(
    State(app): State<AppState>,
) -> (StatusCode, Json<ApiResponse<Vec<VowifiCallStatus>>>) {
    match app.vowifi_tunnel_manager.call_statuses().await {
        Ok(calls) => (
            StatusCode::OK,
            Json(ApiResponse::success_with_message(
                "VoWiFi IMS call and RTP transport status retrieved",
                calls,
            )),
        ),
        Err(error) => (
            StatusCode::SERVICE_UNAVAILABLE,
            Json(ApiResponse::<Vec<VowifiCallStatus>>::error(error)),
        ),
    }
}

/// POST /api/vowifi/call/audio/play
pub async fn play_vowifi_call_audio_handler(
    State(app): State<AppState>,
    Json(payload): Json<VowifiCallAudioPlayRequest>,
) -> (StatusCode, Json<ApiResponse<VowifiCallAudio>>) {
    match app.vowifi_tunnel_manager.play_call_audio(&payload).await {
        Ok(audio) => (
            StatusCode::OK,
            Json(ApiResponse::success_with_message(
                "VoWiFi call audio playback completed",
                audio,
            )),
        ),
        Err(error) => (
            StatusCode::BAD_REQUEST,
            Json(ApiResponse::<VowifiCallAudio>::error(error)),
        ),
    }
}

/// POST /api/vowifi/call/audio/recording
pub async fn get_vowifi_call_recording_handler(
    State(app): State<AppState>,
    Json(payload): Json<VowifiCallAudioRequest>,
) -> (StatusCode, Json<ApiResponse<VowifiCallAudio>>) {
    match app.vowifi_tunnel_manager.call_recording(&payload).await {
        Ok(audio) => (
            StatusCode::OK,
            Json(ApiResponse::success_with_message(
                "VoWiFi call recording retrieved",
                audio,
            )),
        ),
        Err(error) => (
...
/// POST /api/vowifi/call/audio/stats
pub async fn get_vowifi_call_audio_stats_handler(
    State(app): State<AppState>,
    Json(payload): Json<VowifiCallAudioRequest>,
) -> (StatusCode, Json<ApiResponse<VowifiCallAudio>>) {
    match app.vowifi_tunnel_manager.call_audio_stats(&payload).await {
        Ok(audio) => (
            StatusCode::OK,
            Json(ApiResponse::success_with_message(
                "VoWiFi call audio statistics retrieved",
                audio,
            )),
        ),
        Err(error) => (
