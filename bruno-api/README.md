...
### 基础信息接口
- **get_modems.bru** - 获取全部 ModemManager 模块及端口、网卡和状态
...
- **get_proxies.bru** - 获取蜂窝出口代理实例和运行统计
- **post_proxy_create.bru** - 创建绑定指定模块的代理实例
...
- **get_ims_status.bru** - 获取 IMS / VoWiFi 驻网状态
- **post_ims_register.bru** - 启用基带并发起 IMS 驻网（不发起通话）
- **get_vowifi_tunnel_status.bru** - 获取真实 ePDG SWu 隧道状态
- **post_vowifi_tunnel_start.bru** - 为所选模块建立 IKEv2/EAP-AKA SWu 隧道
- **post_vowifi_tunnel_stop.bru** - 优雅停止 SWu 隧道
- **get_vowifi_verification_status.bru** - 获取 SWu、P-CSCF、IMS 和短信分层验证结论
- **post_vowifi_sms_verify.bru** - 检查 VoWiFi 短信发送或接收路径证据
...
除 `/api/modems` 外，modem 相关接口可携带 `X-SimAdmin-Modem-Id` 请求头。该值使用 `/api/modems` 返回的 `id`；未携带时保持兼容并选择当前枚举中的第一台模块。

| 方法 | 端点 | 说明 |
|------|------|------|
| GET | `/api/health` | 健康检查 |
| GET | `/api/modems` | 获取全部模块及其稳定标识、端口、网卡与状态 |
...
| GET | `/api/proxies` | 获取代理实例与流量统计 |
| POST | `/api/proxies` | 创建代理实例 |
| PUT | `/api/proxies/:id` | 更新代理实例 |
| DELETE | `/api/proxies/:id` | 删除代理实例 |
| POST | `/api/proxies/:id/start` | 启动代理实例并绑定模块网卡 |
| POST | `/api/proxies/:id/stop` | 停止代理实例 |
...
| GET | `/api/ims/status` | 获取 IMS / VoWiFi 驻网状态（只读） |
| POST | `/api/ims/register` | 启用基带并发起 IMS 驻网（不发起通话） |
| GET | `/api/vowifi/tunnel/status` | 获取 ePDG SWu helper、Child SA、隧道地址与 P-CSCF |
| POST | `/api/vowifi/tunnel/start` | 为所选模块启动真实 IKEv2/EAP-AKA SWu 会话 |
| POST | `/api/vowifi/tunnel/stop` | 优雅停止 SWu 会话并清理 Child SA |
| GET | `/api/vowifi/verification/status` | 获取严格的 VoWiFi 启用与短信分层验证结论 |
| POST | `/api/vowifi/sms/verify` | 检查 send/receive 方向的 SMS-over-IMS 路径证据 |
