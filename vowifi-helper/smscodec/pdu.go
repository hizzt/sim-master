...
func ParseRPData(body []byte) (byte, []byte, error) {
	rpMR, _, _, tpduBytes, err := ParseRPDataWithAddresses(body)
	return rpMR, tpduBytes, err
}
...
func ParseRPDataWithAddresses(body []byte) (byte, string, string, []byte, error) {
	...
	if body[0] != 0x00 && body[0] != 0x01 {
		return 0, "", "", nil, fmt.Errorf("非 RP-DATA: mti=0x%02x", body[0])
	}
	i := 0
	i++
	rpMr := body[i]
	...
	if end != len(body) {
		return 0, oa, da, nil, fmt.Errorf("RP-DATA 含尾随数据")
	}
	tpduBytes := append([]byte(nil), body[i:end]...)
...
func DecodeDeliverTPDU(tpduBytes []byte) (sender string, text string, ts time.Time, concat ConcatInfo, err error) {
	...
	if err != nil {
		return decodeDeliverTPDUFallback(tpduBytes)
	}
	...
	if t.SmsType() != tpdu.SmsDeliver {
		return "", "", time.Time{}, ConcatInfo{}, fmt.Errorf("TPDU is %s, not SMS-DELIVER", t.SmsType())
	}
