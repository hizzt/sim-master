...
func TestBuildSubmitTPDUsCanRequestStatusReport(t *testing.T) {
	tpdus, _, err := BuildSubmitTPDUsWithOptions("+8615556250521", "hello", SubmitOptions{
		Encoding:            SMSEncodingAuto,
		RequestStatusReport: true,
	})
	if err != nil {
		t.Fatalf("BuildSubmitTPDUsWithOptions() error = %v", err)
	}
	if len(tpdus) != 1 || len(tpdus[0]) == 0 {
		t.Fatalf("TPDUs=%x", tpdus)
	}
	if tpdus[0][0]&byte(tpdu.FoSRR) == 0 {
		t.Fatalf("first octet=0x%02x, TP-SRR is not set", tpdus[0][0])
	}
}
