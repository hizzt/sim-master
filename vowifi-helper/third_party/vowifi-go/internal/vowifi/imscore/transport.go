...
func newRegisterSIPStack(cfg Config, conn net.Conn, swu voiceclient.SWUTCPDialer, localPort int) (*sipgo.UserAgent, *sipgo.Client, error) {
	...
	return newSIPStack(cfg, dialer, swu, localPort, false)
}

func newSecureRegisterSIPStack(cfg Config, conn *ipsec3gpp.SecureChannelConn, advertisedPort int) (*sipgo.UserAgent, *sipgo.Client, error) {
	localPort := 5060
	if advertisedPort > 0 {
		localPort = advertisedPort
	} else if conn != nil && conn.LocalAddr() != nil {
	...
	return newSIPStack(cfg, dialer, nil, localPort, false)
}

func newSIPStack(cfg Config, dialer *fixedConnDialer, swu voiceclient.SWUTCPDialer, localPort int, advertiseRPort bool) (*sipgo.UserAgent, *sipgo.Client, error) {
