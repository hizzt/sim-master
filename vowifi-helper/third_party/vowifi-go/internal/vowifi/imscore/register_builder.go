...
// buildSelectedSecurityClient renders the mechanism actually selected from
// Security-Server.  Mature IMS stacks advertise the capability list on the
// initial REGISTER, then send the negotiated association as a single
// Security-Client entry on the protected REGISTER.
func buildSelectedSecurityClient(selected *imsheaders.SecurityOffer, spiC, spiS uint32, portC, portS int) string {
	if selected == nil {
		return ""
	}
	prot := strings.TrimSpace(selected.Prot)
	if prot == "" {
		prot = "esp"
	}
	...
	return policy.BuildSecurityClientHeader(
		policy.IMSRegisterTemplate{SecurityClientMechanisms: []policy.IPSec3GPPSecurityMechanism{{
			Alg:  selected.Alg,
			EAlg: selected.EAlg,
			Prot: prot,
			Mode: mode,
