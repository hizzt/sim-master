...
func TestAttemptRegisterModePreservesExplicitRejectContract(t *testing.T) {
	reject := &registrarAttemptError{pcscf: "pcscf-a", statusCode: sip.StatusForbidden, reason: "Forbidden"}
	if reject.Error() != "unexpected initial REGISTER response: 403 Forbidden" {
		t.Fatalf("reject error = %q", reject.Error())
	}
}
