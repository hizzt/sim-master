...
func SetupService(imsCfg IMSConfig, network IMSNetwork, in StartSessionInput) (*Service, error) {
	...
	// Generate the instance once per IMS service so REGISTER and every dialog
	// request identify the same feature-capability binding.
	internal.SIPInstanceURN = resolveStableSIPInstance(internal)
	if installer, ok := network.(IPSecInstaller); ok {
		internal.IPSecInstaller = installer
	}
