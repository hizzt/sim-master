...
func (s *Service) registerRawWithCandidate(ctx context.Context, candidate registerAttemptCandidate, transportMode string, attemptIndex int) registerTransportAttemptResult {
	...
	attemptCtx := ctx
	cancel := func() {}
	if !initialRejectFallbackEnabled(attemptCfg) {
...
func (s *Service) attemptRegisterMode(ctx context.Context, transportMode string, candidates []registerAttemptCandidate) (*registerResult, error) {
	...
	var lastExplicitReject *registrarAttemptError
	for i, candidate := range candidates {
		...
		if attempt.err != nil {
			var attemptErr *registrarAttemptError
			if errors.As(attempt.err, &attemptErr) {
				lastExplicitReject = &registrarAttemptError{
					pcscf:      attemptErr.pcscf,
					statusCode: attemptErr.statusCode,
					reason:     attemptErr.reason,
				}
				if shouldAdvanceRegistrarForNextRetry(attemptErr.statusCode, attemptErr.reason, hasMore) {
			...
			if lastExplicitReject != nil {
				logger.Warn("IMS REGISTER transport exhausted after timeout; preserving explicit SIP rejection",
					logger.String("trace_id", strings.TrimSpace(s.cfg.TraceID)),
					logger.String("device_id", strings.TrimSpace(s.cfg.DeviceID)),
					logger.Int("status", lastExplicitReject.statusCode),
					logger.String("reason", lastExplicitReject.reason),
					logger.String("last_transport_error", attempt.err.Error()))
				return nil, lastExplicitReject
			}
			return nil, attempt.err
		}
	}
