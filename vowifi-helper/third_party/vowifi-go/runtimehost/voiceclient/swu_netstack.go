...
// SWUUDPDialer opens connected or listening UDP endpoints on the same
// userspace netstack as IMS TCP. A second netstack must not consume the shared
// PacketDataplane because InnerPackets has a single consumer.
type SWUUDPDialer interface {
	DialContextUDP(ctx context.Context, localIP net.IP, localPort int, remoteIP net.IP, remotePort int) (net.Conn, error)
	ListenPacketUDP(ctx context.Context, localIP net.IP, localPort int) (net.PacketConn, error)
}
...
func (n *swuNetstack) DialContextUDP(ctx context.Context, localIP net.IP, localPort int, remoteIP net.IP, remotePort int) (net.Conn, error) {
	...
	localIP, err := n.validateLocalIP(localIP)
	if err != nil {
		return nil, err
	}
	if remoteIP == nil || remotePort <= 0 || remotePort > 65535 {
		return nil, fmt.Errorf("voiceclient: invalid UDP remote %s:%d", remoteIP, remotePort)
	}
	networkProto := networkProtocolForIP(remoteIP)
	if networkProto == 0 || networkProto != networkProtocolForIP(localIP) {
		return nil, fmt.Errorf("voiceclient: unsupported UDP IP pair %s -> %s", localIP, remoteIP)
	}
	if localPort < 0 || localPort > 65535 {
		return nil, fmt.Errorf("voiceclient: invalid UDP local port %d", localPort)
	}
	localAddr := tcpip.FullAddress{
		NIC:  swuNetstackNICID,
		Addr: addrFromNetIP(localIP),
	...
	if err != nil {
		return nil, fmt.Errorf("voiceclient: SWu userspace UDP dial %s:%d: %w", remoteIP, remotePort, err)
	}
	return conn, nil
}

func (n *swuNetstack) ListenPacketUDP(ctx context.Context, localIP net.IP, localPort int) (net.PacketConn, error) {
	if err := contextError(ctx); err != nil {
		return nil, err
	}
	localIP, err := n.validateLocalIP(localIP)
	...
	if networkProto == 0 {
		return nil, fmt.Errorf("voiceclient: unsupported UDP listen IP %s", localIP)
	}
	localAddr := tcpip.FullAddress{
		NIC:  swuNetstackNICID,
		Addr: addrFromNetIP(localIP),
	...
	if err != nil {
		return nil, fmt.Errorf("voiceclient: SWu userspace UDP listen %s:%d: %w", localIP, localPort, err)
	}
	return conn, nil
}
...
func (n *swuNetstack) validateLocalIP(localIP net.IP) (net.IP, error) {
	if localIP == nil || localIP.IsUnspecified() {
		localIP = n.localIP
	}
	if localIP == nil || !localIP.Equal(n.localIP) {
		return nil, fmt.Errorf("voiceclient: UDP local IP %v is not the SWu address %s", localIP, n.localIP)
	}
	return append(net.IP(nil), n.localIP...), nil
}
