...
func TestDialogContactHeaderReusesRegisterFeaturesWithoutExpires(t *testing.T) {
	cfg := Config{
	...
	header := cfg.dialogContactHeader(DefaultGBEERegisterProfile(), "urn:uuid:registered", "")
	if strings.Contains(strings.ToLower(header), ";expires=") {
		t.Fatalf("dialog Contact retained REGISTER expires: %q", header)
	}
	for _, want := range []string{"subscriber@", ";audio", "+g.3gpp.icsi-ref", "urn:uuid:registered"} {
		if !strings.Contains(header, want) {
			t.Fatalf("dialog Contact %q missing registered feature %q", header, want)
		}
	}
}
