...
func (c *Client) registerOnceAttempt(ctx context.Context) error {
	...
	var refreshedAuth *DigestAuthSession
	for round := 0; round < maxChallengeRounds && (res.StatusCode == 401 || res.StatusCode == 407); round++ {
		logger.Info("IMS REGISTER answering challenge",
			...
			logger.Int("status", res.StatusCode))
		var roundAuth *DigestAuthSession
		res, req, roundAuth, err = c.answerChallenge(ctx, req, res)
		if err != nil {
			return fmt.Errorf("challenge round %d: %w", round+1, err)
		}
		if roundAuth != nil {
			refreshedAuth = roundAuth
		}
	...
	c.commitRegisterSuccess(res, refreshedAuth)
...
func (c *Client) answerChallenge(ctx context.Context, prevReq *sip.Request, prevRes *sip.Response) (*sip.Response, *sip.Request, *DigestAuthSession, error) {
	...
	if err != nil {
		return nil, nil, nil, fmt.Errorf("simauth.ComputeDigest: %w", err)
	}
	var refreshedAuth *DigestAuthSession
	if !result.SyncFailure && len(result.AKAResult.RES) > 0 {
		refreshedAuth = newVoiceDigestAuthSession(chal, result.AKAResult.RES, c.cfg.PrivateID, prevRes)
	}
	...
	newReq.AppendHeader(sip.NewHeader(authorizationHeader, result.Header))
	...
	return res, newReq, refreshedAuth, nil
}

// recoverRegisteredFlow establishes a fresh REGISTER transaction on the
// currently usable transport and atomically refreshes registration-derived
// routing, identity, and digest state. Callers still retry their own request
// exactly once after this succeeds.
func (c *Client) recoverRegisteredFlow(ctx context.Context) error {
	logger.Warn("IMS registered flow recovery started",
		logger.String("trace_id", strings.TrimSpace(c.cfg.TraceID)),
		logger.String("device_id", strings.TrimSpace(c.cfg.DeviceID)))
	if err := c.registerWithResync(ctx); err != nil {
		return fmt.Errorf("voiceclient: recover registered flow: %w", err)
	}
	logger.Info("IMS registered flow recovery completed",
		logger.String("trace_id", strings.TrimSpace(c.cfg.TraceID)),
		logger.String("device_id", strings.TrimSpace(c.cfg.DeviceID)),
		logger.Bool("preemptive_auth", c.preemptiveAuthSnapshot() != nil))
	return nil
}

func (c *Client) commitRegisterSuccess(res *sip.Response, auth *DigestAuthSession) {
	if res == nil {
		return
	}
	associated := AssociatedPublicIdentities(res)
	if len(associated) > 0 {
		c.cfg.PreferredIdentity = AssociatedPreferredIdentity(associated)
	}
	if telephoneIdentity := AssociatedTelephoneIdentity(associated); telephoneIdentity != "" {
		c.cfg.SMSPreferredIdentity = telephoneIdentity
	}
	logger.Info("IMS REGISTER associated identities received",
		logger.String("trace_id", strings.TrimSpace(c.cfg.TraceID)),
		logger.Int("associated_identity_count", len(associated)),
		logger.String("associated_identity_schemes", associatedIdentitySchemes(associated)),
		logger.Bool("preferred_identity_selected", strings.TrimSpace(c.cfg.PreferredIdentity) != ""))
	...
	if len(serviceRoutes) > 0 {
		c.cfg.ServiceRoutes = serviceRoutes
	}
	if auth == nil {
		return
	}
	if nextNonce := voiceNextNonceFromAuthenticationInfo(res); nextNonce != "" {
		auth.Challenge.Nonce = nextNonce
	}
	c.setPreemptiveAuth(auth)
}

func newVoiceDigestAuthSession(chal *digest.Challenge, akaResponse []byte, username string, res *sip.Response) *DigestAuthSession {
...
func (c *Client) setPreemptiveAuth(auth *DigestAuthSession) {
	c.digestMu.Lock()
	defer c.digestMu.Unlock()
	c.cfg.PreemptiveAuth = cloneVoiceDigestAuthSession(auth)
	c.digestNC = 0
}

func (c *Client) preemptiveAuthSnapshot() *DigestAuthSession {
	c.digestMu.Lock()
	defer c.digestMu.Unlock()
	return cloneVoiceDigestAuthSession(c.cfg.PreemptiveAuth)
}

func cloneVoiceDigestAuthSession(auth *DigestAuthSession) *DigestAuthSession {
	if auth == nil {
		return nil
	}
	copy := *auth
	copy.Challenge.Domain = append([]string(nil), auth.Challenge.Domain...)
	copy.Challenge.QOP = append([]string(nil), auth.Challenge.QOP...)
	copy.AKAResponse = append([]byte(nil), auth.AKAResponse...)
	return &copy
}

func voiceNextNonceFromAuthenticationInfo(res *sip.Response) string {
	if res == nil {
		return ""
	}
	for _, header := range res.GetHeaders("Authentication-Info") {
		...
		for _, field := range strings.Split(header.Value(), ",") {
			...
			if ok && strings.EqualFold(strings.TrimSpace(name), "nextnonce") {
...
func (c *Client) selectDigestChallenge(prevRes *sip.Response) (*digest.Challenge, error) {
	...
	// Some IMS cores return Proxy-Authenticate with 401 instead of the
	// standards-compliant 407. Follow the challenge header that is actually
	// present so MESSAGE authentication remains interoperable with them.
	if len(headers) == 0 {
		headers = prevRes.GetHeaders("Proxy-Authenticate")
	}
	if len(headers) == 0 {
		return nil, fmt.Errorf("%d response with no authenticate header (headers: %s)", prevRes.StatusCode, responseHeaderNames(prevRes))
	}
...
func responseHeaderNames(res *sip.Response) string {
	if res == nil {
		return "none"
	}
...
func (c *Client) doTransaction(ctx context.Context, req *sip.Request, opts ...sipgo.ClientRequestOption) (*sip.Response, error) {
	if c.transactionHook != nil {
		return c.transactionHook(ctx, req, opts...)
	}
