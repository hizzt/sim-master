...
func (i *Instance) StartSignalingCall(ctx context.Context, phoneNumber string) (voiceclient.SignalingCallStatus, error) {
	service := i.Service()
	controller, ok := service.(voiceclient.SignalingCallController)
	if !ok || controller == nil {
		return voiceclient.SignalingCallStatus{}, errors.New("runtimehost: IMS signaling call controller is unavailable")
	}
	return controller.StartSignalingCall(ctx, phoneNumber)
}

func (i *Instance) HangupSignalingCall(ctx context.Context, callID string) (voiceclient.SignalingCallStatus, error) {
	service := i.Service()
	controller, ok := service.(voiceclient.SignalingCallController)
	if !ok || controller == nil {
		return voiceclient.SignalingCallStatus{}, errors.New("runtimehost: IMS signaling call controller is unavailable")
	}
	return controller.HangupSignalingCall(ctx, callID)
}

func (i *Instance) SignalingCallStatuses() []voiceclient.SignalingCallStatus {
	service := i.Service()
	controller, ok := service.(voiceclient.SignalingCallController)
	if !ok || controller == nil {
		return []voiceclient.SignalingCallStatus{}
	}
	return controller.SignalingCallStatuses()
}

func (i *Instance) PlaySignalingAudio(ctx context.Context, callID string, input voiceclient.SignalingAudioPlayback) (voiceclient.SignalingAudioStats, error) {
	service := i.Service()
	controller, ok := service.(voiceclient.SignalingAudioController)
	if !ok || controller == nil {
		return voiceclient.SignalingAudioStats{}, errors.New("runtimehost: IMS signaling audio controller is unavailable")
	}
	return controller.PlaySignalingAudio(ctx, callID, input)
}

func (i *Instance) SignalingAudioRecording(callID, format string) (voiceclient.SignalingAudioRecording, error) {
	service := i.Service()
	controller, ok := service.(voiceclient.SignalingAudioController)
	if !ok || controller == nil {
		return voiceclient.SignalingAudioRecording{}, errors.New("runtimehost: IMS signaling audio controller is unavailable")
	}
	return controller.SignalingAudioRecording(callID, format)
}

func (i *Instance) SignalingAudioStats(callID string) (voiceclient.SignalingAudioStats, error) {
	service := i.Service()
	controller, ok := service.(voiceclient.SignalingAudioController)
	if !ok || controller == nil {
		return voiceclient.SignalingAudioStats{}, errors.New("runtimehost: IMS signaling audio controller is unavailable")
	}
	return controller.SignalingAudioStats(callID)
}
...
func (i *Instance) State() State {
	i.mu.Lock()
	state := i.state
	dataplane := i.dataplane
	i.mu.Unlock()
	if stats, ok := dataplane.(interface{ InnerPacketCounts() (uint64, uint64) }); ok {
		state.InnerTXPackets, state.InnerRXPackets = stats.InnerPacketCounts()
	}
	return state
}
...
func (i *Instance) Obs() map[string]interface{} {
	...
	obs := map[string]interface{}{
		...
		"inner_tx_packets": st.InnerTXPackets,
		"inner_rx_packets": st.InnerRXPackets,
...
func (i *Instance) ProbeSIPOptions(ctx context.Context, input voiceclient.SIPOptionsProbeRequest) (voiceclient.SIPOptionsProbeResult, error) {
	service := i.Service()
	prober, ok := service.(voiceclient.SIPOptionsProber)
	if !ok || prober == nil {
		return voiceclient.SIPOptionsProbeResult{}, errors.New("runtimehost: IMS SIP OPTIONS probe is unavailable")
	}
	return prober.ProbeSIPOptions(ctx, input)
}

func (i *Instance) ProbeSIPEmptyMessage(ctx context.Context, input voiceclient.SIPOptionsProbeRequest) (voiceclient.SIPOptionsProbeResult, error) {
	service := i.Service()
	prober, ok := service.(voiceclient.SIPEmptyMessageProber)
	if !ok || prober == nil {
		return voiceclient.SIPOptionsProbeResult{}, errors.New("runtimehost: IMS SIP MESSAGE probe is unavailable")
	}
	return prober.ProbeSIPEmptyMessage(ctx, input)
}
...
func (i *Instance) learnLocalNumber(ctx context.Context, number, source string) {
	if i == nil {
		return
	}
	...
	i.mu.Lock()
	if i.state.PhoneNumber == number {
		i.mu.Unlock()
		return
	}
	i.state.PhoneNumber = number
	...
	if dispatcher != nil {
		dispatcher.Dispatch(ctx, eventhost.LocalNumberLearned{
			DevID:  deviceID,
			Number: number,
...
func Start(ctx context.Context, req StartRequest) (*Instance, error) {
	...
	inst := &Instance{
		...
		inboundSMSHandler:       req.InboundSMSHandler,
		inboundSMSResultHandler: req.InboundSMSResultHandler,
...
func (i *Instance) runStagedPipeline(ctx context.Context, req StartRequest) {
	...
	i.updateState(func(s *State) {
		s.TunnelReady = true
		s.EPDGIP = epdgIP
		s.TunnelIPv4 = ipString(snapshot.IPv4)
		s.TunnelIPv6 = ipString(snapshot.IPv6)
		s.PCSCFv4 = ipStrings(snapshot.PCSCFv4)
		s.PCSCFv6 = ipStrings(snapshot.PCSCFv6)
	...
	voiceCfg := voiceclient.Config{
		...
		InboundSMSHandler:       i.inboundSMSHandler,
		InboundSMSResultHandler: i.inboundSMSResultHandler,
	}
	...
	imsTemplate.LocationHeaderMode = strings.TrimSpace(voiceCfg.RegisterProfile.LocationHeaderMode)
	imsTemplate.EnableInitialRejectFallback = strings.TrimSpace(voiceCfg.RegisterProfile.VariantSet) != ""
	...
	if phoneNumber, ok := svc.Status()["phone_number"].(string); ok {
		i.learnLocalNumber(pipelineCtx, phoneNumber, "register")
	}
	...
	i.notifyObservers(ctx)
	// The pipeline context owns the IMS transport runtime and port-s listener.
	// Keep it alive after staged startup; Instance.Stop cancels it explicitly.
	pipelineReady = true
}
