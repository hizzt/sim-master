...
//   - success: HA1 = MD5(username:realm:RES) -- RES is the binary password
//     specified by Digest AKAv1 -- HA2/response otherwise identical to plain
//     RFC 2617
...
type Result struct {
	...
	// AKAResult is the result from the single SIM authentication call used
	// to build Header. IMS sec-agree callers reuse CK/IK from this value
	// instead of submitting the same RAND/AUTN to the SIM a second time.
	AKAResult sim.AKAResult
...
func ComputeDigest(provider sim.AKAProvider, chal *digest.Challenge, opts digest.Options) (Result, error) {
	...
	switch {
	case akaErr == nil:
		header, err := ComputeDigestWithAKAResponse(chal, akaResult.RES, opts)
		if err != nil {
			return Result{}, err
		}
		return Result{Header: header, AKAResult: akaResult}, nil
	...
	case errors.Is(akaErr, sim.ErrSyncFailure):
		...
		return Result{Header: header, AKAResult: akaResult, SyncFailure: true}, nil
...
// ComputeDigestWithAKAResponse reuses the binary RES from an already
// successful IMS-AKA exchange to authorize another request in the same
// Digest protection space. It never calls the SIM again.
func ComputeDigestWithAKAResponse(chal *digest.Challenge, akaResponse []byte, opts digest.Options) (string, error) {
	if chal == nil {
		return "", errors.New("simauth: nil challenge")
	}
	if len(akaResponse) == 0 {
		return "", errors.New("simauth: empty AKA response")
	}
	wireAlgorithm := chal.Algorithm
	if wireAlgorithm == "" {
		wireAlgorithm = "AKAv1-MD5"
	...
	mathChal := *chal
	mathChal.Algorithm = "MD5"
	// RFC 4169 section 3 distinguishes AKAv1's binary RES password from
	// AKAv2's base64 representation. Preserve the raw bytes here.
	opts.Password = string(akaResponse)
	cred, err := digest.Digest(&mathChal, opts)
	if err != nil {
		return "", fmt.Errorf("simauth: compute digest with cached AKA response: %w", err)
	}
	cred.Algorithm = wireAlgorithm
