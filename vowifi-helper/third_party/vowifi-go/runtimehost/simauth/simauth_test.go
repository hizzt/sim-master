...
func TestComputeDigestSuccess(t *testing.T) {
	...
	if string(result.AKAResult.CK) != string(bytesOf(0x21, 16)) || string(result.AKAResult.IK) != string(bytesOf(0x22, 16)) {
		t.Fatalf("AKAResult = %+v", result.AKAResult)
	}
	...
	// side would (binary RES password, math done as plain MD5) and check
	...
	hexPassword, err := digest.Digest(&mathChal, digest.Options{
		Method:   opts.Method,
		URI:      opts.URI,
		Username: opts.Username,
		Password: "1112131415161718",
	})
	if err != nil {
		t.Fatalf("hex-password digest.Digest() error = %v", err)
	}
	if strings.Contains(result.Header, `response="`+hexPassword.Response+`"`) {
		t.Fatalf("Header = %q, incorrectly used hexadecimal RES text", result.Header)
	}
}
...
func TestComputeDigestSyncFailure(t *testing.T) {
	...
	if string(result.AKAResult.AUTS) != string(testAUTS) {
		t.Fatalf("AUTS = %x, want %x", result.AKAResult.AUTS, testAUTS)
	}
