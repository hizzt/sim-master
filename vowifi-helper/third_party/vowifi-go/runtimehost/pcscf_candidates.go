...
func resolvePCSCFCandidates(snapshot swuSnapshot, override string, localIP net.IP) []string {
	if v := strings.TrimSpace(override); v != "" {
		// An explicit carrier P-CSCF is authoritative. It exists for the case
		// where IKE assigns a silent or unreachable node, so do not append the
		// discovered addresses and accidentally route REGISTER back to them.
		return []string{v}
	}
