...
// InboundSMS carries one network-originated RP-DATA envelope received in a
// SIP MESSAGE. Decoding the embedded TPDU remains the embedding application's
// responsibility, just like outbound TPDU encoding.
...
// InboundSMSReply is the RP-ACK or RP-ERROR body that must be returned to the
// SMSC in a separate SIP MESSAGE.
type InboundSMSReply struct {
	Body []byte
	// ContentType is the MIME type for an asynchronous RP reply. Empty means
	// the raw 3GPP SMS body; voiceclient fills this for CPIM-wrapped inbound
	// MESSAGEs so the network sees the same envelope on the way back.
	ContentType string
}

type InboundSMSHandler func(InboundSMS) (InboundSMSReply, error)
type InboundSMSResultHandler func(InboundSMS, InboundSMSReply, int, error)
