...
type Preset struct {
	...
	// IMSLocationHeaderMode controls P-Access-Network-Info and
	// Cellular-Network-Info in IMS REGISTER requests.
	IMSLocationHeaderMode string `json:"ims_location_header_mode,omitempty"`
	...
	// SMSServicesURI optionally pins the IP-SM-GW service-centre PSI used as
	// the SIP MESSAGE Request-URI.  It is intentionally separate from the
	// numeric SMSC address carried inside RP-DATA, because SIM EF-SMSP and the
	// operator's public PSI are not always the same value.
	SMSServicesURI string `json:"sms_service_center_uri,omitempty"`
