...
func TestMODP2048KeyPairAndSharedSecret(t *testing.T) {
	private, public, err := modp2048KeyPair([]byte{2}, nil)
	if err != nil {
		t.Fatalf("modp2048KeyPair() error = %v", err)
	}
	if len(public) != 256 || public[255] != 4 {
		t.Fatalf("public length=%d tail=%x", len(public), public[len(public)-1:])
	}
	peer := make([]byte, 256)
	peer[255] = 8
	shared, err := modp2048SharedSecret(private, peer)
	if err != nil {
		t.Fatalf("modp2048SharedSecret() error = %v", err)
	}
	if len(shared) != 256 || shared[255] != 64 {
		t.Fatalf("shared length=%d tail=%x", len(shared), shared[len(shared)-1:])
	}
}
