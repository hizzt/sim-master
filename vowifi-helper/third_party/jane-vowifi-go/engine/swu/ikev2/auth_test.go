...
func TestBuildIKEAuthInitialPayloadsInjectsSPIIntoCustomChildSA(t *testing.T) {
	custom := DefaultESPProposal(nil)
	payloads, err := BuildIKEAuthInitialPayloads(AuthConfig{
		InitiatorID: Identity{Type: IDRFC822Addr, Data: []byte("0imsi@example.invalid")},
		ChildSA:     custom,
	...
	for _, payload := range payloads {
		...
		sa, parseErr := ParseSecurityAssociation(payload.Body)
		if parseErr != nil {
			t.Fatalf("ParseSecurityAssociation() error = %v", parseErr)
		}
		if len(sa.Proposals) != 1 || !bytes.Equal(sa.Proposals[0].SPI, []byte{1, 2, 3, 4}) {
			t.Fatalf("child SA SPI=%x", sa.Proposals[0].SPI)
		}
		return
	}
	t.Fatal("child SA payload not found")
}

func TestInitiatorEAPAuthDataUsesMSKKeyPadAndSignedOctets(t *testing.T) {
	profile, err := KeyMaterialProfileFromSA(SecurityAssociation{Proposals: []Proposal{{
		Number: 1, ProtocolID: ProtocolIKE, Transforms: []Transform{
	...
	init := InitResult{RequestBytes: []byte("ike-sa-init"), NonceR: []byte("nonce-r")}
	keys := IKEKeys{Profile: profile, SKPi: bytes.Repeat([]byte{0x11}, profile.PRFKeyLength)}
	id := Identity{Type: IDRFC822Addr, Data: []byte("0imsi@nai.example")}
	msk := bytes.Repeat([]byte{0x22}, eapaka.KeyLengthMSK)
	got, err := initiatorEAPAuthData(init, keys, id, msk)
	if err != nil {
		t.Fatal(err)
	}
	idPayload, _ := IdentityPayload(PayloadIDi, id)
	idHash, _ := PRF(profile.PRF, keys.SKPi, idPayload.Body)
	signed := append(append(append([]byte(nil), init.RequestBytes...), init.NonceR...), idHash...)
	keyPad, _ := PRF(profile.PRF, msk, []byte("Key Pad for IKEv2"))
	want, _ := PRF(profile.PRF, keyPad, signed)
