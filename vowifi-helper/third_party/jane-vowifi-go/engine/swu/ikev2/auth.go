...
func RunIKE_AUTH_Full(ctx context.Context, cfg FullAuthConfig) (FullAuthResult, error) {
	...
	for i := 0; i < maxFullAuthEAPExchanges; i++ {
		...
		if next.Code == eapaka.CodeSuccess {
			...
			requestBytes, responseBytes, responseInner, err := runIKEAuthEAPFinal(ctx, cfg, out.EAPKeys, out.NextMessageID)
			if err != nil {
				return FullAuthResult{}, err
			}
			out.FinalRequestBytes = append([]byte(nil), requestBytes...)
			out.FinalResponseBytes = append([]byte(nil), responseBytes...)
			out.FinalResponseInner = clonePayloads(responseInner)
			out.NextMessageID++
			if child, ok, err := parseChildSAIfPresent(cfg.Init, responseInner, localChildSPI, out.NextMessageID, offeredChildSA, offeredTSi, offeredTSr); err != nil {
				return FullAuthResult{}, err
			} else if ok {
				out.ChildSA = &child
				return out, nil
			}
			return out, fmt.Errorf("%w: final EAP AUTH response without CHILD_SA", ErrInvalidAuthResponse)
		}
...
func BuildIKEAuthInitialPayloads(cfg AuthConfig) ([]Payload, error) {
	...
	if len(childSA.Proposals) == 0 {
	...
	} else if len(childSA.Proposals[0].SPI) == 0 {
		spi := append([]byte(nil), cfg.ChildSPI...)
		if len(spi) == 0 {
			random := cfg.Random
			if random == nil {
		...
		childSA = cloneSecurityAssociation(childSA)
		childSA.Proposals[0].SPI = spi
	}
...
func runIKEAuthEAPFinal(ctx context.Context, cfg FullAuthConfig, eapKeys eapaka.Keys, messageID uint32) ([]byte, []byte, []Payload, error) {
	keys := cfg.Keys
	if keys.Profile.RequiredLength() == 0 {
		keys = cfg.Init.Keys
	}
	...
	authData, err := initiatorEAPAuthData(cfg.Init, keys, cfg.InitiatorID, eapKeys.MSK)
	if err != nil {
		return nil, nil, nil, err
	}
	authPayload := Payload{
		Type: PayloadAUTH,
		Body: append([]byte{authMethodSharedKeyMIC, 0, 0, 0}, authData...),
	}
	iv, err := authIV(cfg.Random, keys.Profile, nil)
	if err != nil {
		return nil, nil, nil, err
	}
	_, requestBytes, err := ProtectMessage(authHeader(cfg.Init, messageID, true), keys, true, []Payload{authPayload}, iv)
	if err != nil {
		return nil, nil, nil, err
	}
	...
	_, responseInner, err := unprotectAuthResponse(responseBytes, cfg.Init, keys, messageID)
	if err != nil {
		return nil, nil, nil, err
	}
	return requestBytes, responseBytes, responseInner, nil
}

func initiatorEAPAuthData(init InitResult, keys IKEKeys, initiatorID Identity, msk []byte) ([]byte, error) {
	if len(init.RequestBytes) == 0 || len(init.NonceR) == 0 || len(keys.SKPi) == 0 || len(msk) == 0 {
		return nil, fmt.Errorf("%w: incomplete EAP AUTH key material", ErrInvalidAuthConfig)
	}
	idPayload, err := IdentityPayload(PayloadIDi, initiatorID)
	if err != nil {
		return nil, err
	}
	idHash, err := PRF(keys.Profile.PRF, keys.SKPi, idPayload.Body)
	if err != nil {
		return nil, err
	}
	...
	keyPad, err := PRF(keys.Profile.PRF, msk, []byte("Key Pad for IKEv2"))
	if err != nil {
		return nil, err
	}
	return PRF(keys.Profile.PRF, keyPad, signedOctets)
}
...
func authOfferedChildSA(sa SecurityAssociation, localSPI []byte) SecurityAssociation {
	...
	offered := cloneSecurityAssociation(sa)
	if len(offered.Proposals) > 0 && len(offered.Proposals[0].SPI) == 0 && len(localSPI) > 0 {
		offered.Proposals[0].SPI = append([]byte(nil), localSPI...)
	}
	return offered
}
